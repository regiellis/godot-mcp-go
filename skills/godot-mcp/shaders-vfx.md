# Writing gdshader code for VFX and wiring it with the CLI

Authoring `.gdshader` code and wiring it to nodes for game VFX. `game-patterns.md` holds the
**combat-VFX grammar** (the aesthetic recipe of lifetime curves, LUT recolor, erosion, polar remaps);
...
- **`canvas_item`** is 2D. Writes `COLOR` (rgba); reads `TEXTURE`/`UV`/`SCREEN_UV`/`TEXTURE_PIXEL_SIZE`.
  Every 2D sprite, UI, and particle.
- **`spatial`** is 3D. Writes `ALBEDO`/`ALPHA`/`EMISSION`/`NORMAL`… in `fragment()`, moves geometry in
  `vertex()`; reads `NORMAL`/`VIEW`/`VERTEX`/`UV`/`TIME`.
- **`particles`** takes over the GPU particle system when `ParticleProcessMaterial` runs out. `start()`
  seeds a particle, `process()` steps it; writes `TRANSFORM`/`VELOCITY`/`COLOR`, reads
  `DELTA`/`LIFETIME`/`INDEX`.
- **`sky`** / **`fog`** drive a `Sky`'s shader (`sky()` writes `COLOR`) and a `FogVolume`'s
  density/color (`fog()` writes `DENSITY`/`ALBEDO`). Both are niche: procedural skies, volumetric fog.

**Render modes worth knowing** (line 2, `render_mode a, b;`, verified compiling):
- `unshaded` skips all lighting; the pixel is exactly what you write. Base for emitters, UI, flats.
- `blend_add` is additive (brightens what's behind). `render_mode unshaded, blend_add;` is the glow-ish
  sprite: muzzle flashes, lasers, energy. (2D real bloom needs `hdr_2d`, covered in `lighting-2d.md`.)
- `cull_disabled` (spatial) draws back faces too, for flat/thin geometry (grass cards, capes).
...
value is `null` **dies silently**, with nothing animated and no error. Verified live: tweening a fresh
...
- **`uniform`** is a per-material knob, set from CLI/code/inspector. Hints shape it: `uniform float x
  ...
  give a default, since an unset uniform reads as zero.
- **`instance uniform`** gives per-*node* variation on **one shared material**, no duplication.
  `instance uniform vec4 tint : source_color = vec4(1.0);`, then set it per node via the node's own
  method, `set_instance_shader_parameter("tint", …)`. That method lives on **both**
  `GeometryInstance3D` (3D) and `CanvasItem` (2D), so instance uniforms are *not* spatial-only
  (verified live); unavailable in `particles`/`sky`/`fog`. Reach the setter with `editor run-script`;
  they don't enumerate in `get_shader_uniform_list`.
- **`global uniform`** is one value shared by every shader project-wide (wind, time-of-day, player
...
**Hit-flash** mixes the sprite to white by a uniform (`modulate` only multiplies, so it can't whiten a
...
**Dissolve** uses a noise texture + threshold to burn the sprite away with a glowing edge; the
threshold *is* the driver (tween 0→1 to dissolve out).
...
**Outline** samples the four neighbors and draws where the sprite is transparent but a neighbor opaque.
...
**UV scroll** offsets UVs by `TIME` for conveyors, water, lava, and scrolling backgrounds; `TIME`
drives it for free. The texture must import with **Repeat** on (or `repeat_enable` on a sampler variant).
...
**Palette swap (LUT)** indexes a 1-D palette texture by the sprite's red channel; one grayscale sprite
...
```
Point `palette` at a different LUT to swap teams/rarities, with no per-variant art.

## 3D quick hits

**Fresnel rim** is a glow at grazing angles (shields, selection, ghosts); `EMISSION` so it ignores lighting.
...
**Vertex sway (wind)** displaces geometry in `vertex()`, masked by height so roots stay planted (grass, foliage, banners).
...
(verified), so don't treat their absence as a compile error.

- **Full-screen effect**: assign the shader to a `ColorRect` on a top `CanvasLayer` stretched to the
  viewport, and it post-processes everything below (vignette / grade / chromatic aberration).
- **`BackBufferCopy`**: the screen sampler reads the frame *as of the last backbuffer copy*. To read
  ...
  it in draw order to snapshot first. That is the node `game-patterns.md`'s screen-distortion recipe
  leans on.
...
`script validate` compiles **GDScript only**, with nothing for `.gdshader`. But shader compile errors
*are* detectable: assigning code to an orphan `Shader` compiles it synchronously, and two signals
report the result (both established live). Run it before attaching, with no scene and no file:
...
- **Editing `project.godot` for global uniforms**. Use `shader global-add`/`global-set`, since a
  `global uniform` won't compile until the global is registered (verified error above). Same for setting a
...
- **Shared `.tres` material across instances**. A uniform set on a shared `ShaderMaterial` changes
  *every* node using it, so `duplicate()` it or use an `instance uniform`. (`assign-material` already
...
- **`COLOR` vs `ALBEDO`**. `canvas_item` writes `COLOR` (rgba); `spatial` writes `ALBEDO` (rgb) +
  ...
  the wrong output name, so check `shader_type` first.
