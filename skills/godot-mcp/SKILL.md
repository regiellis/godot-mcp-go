...
`export` and `import` are addon group names as well, and both still work: a command name after them (`export list-presets`, `export info`, `import reimport`) routes to the running editor, while anything else runs the local subcommand.
