...
- `calibration_offset_ms` is a **user setting**, not a constant, because hardware audio paths
  differ. Ship a "tap to the beat" calibration screen and store the offset in the save config.
...
## Beatmaps and lyrics are data, so reuse existing formats
...
target note and bucket the absolute error into windows, say ±120 ms perfect, ±300 great,
...
perfect inputs itself) is worth building day one, since it is both the demo/attract mode and the
