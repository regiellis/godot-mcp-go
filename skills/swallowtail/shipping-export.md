...
```
swallowtail export info            # templates installed? export_presets.cfg present?
swallowtail export list-presets    # name, platform, runnable, export_path per preset
swallowtail export project --preset-name "Windows Desktop" --debug=false
```

`export project` returns the headless command line rather than running it, because a Godot 4 editor
plugin cannot export. `swallowtail export "<preset>"` is the local subcommand that runs that command
line for you, and it needs no editor at all.

## The dev-tooling boundary: this addon must never ship

The swallowtail addon is a WebSocket command server with arbitrary-eval and input-injection
surfaces, and its two game-side autoloads stat IPC files every frame. None of that belongs in a
player build.

1. **Export filters.** In the preset: `exclude_filter="addons/swallowtail/*,tools/*"` (plus any
...
```
swallowtail export "Windows Desktop"
```
...
`<project>/.godot/swallowtail-export.log`. Where the CLI is not installed, the raw form is what it
runs for you:
...
- **An export that exits 0 and wrote nothing is the failure to watch for**, and `swallowtail export`
...
- Have CI copy it into place as the first build step, then supply the real values from the
  ...
  - run: swallowtail export "Web" --out build/web/index.html
    env:
      GODOT_SCRIPT_ENCRYPTION_KEY: ${{ secrets.PCK_KEY }}
  ```

- Verify the copy landed before blaming the export. `swallowtail export info` reports
  `"has_export_presets": false` and `swallowtail export list-presets` answers
...
```sh
swallowtail runtime eval --code 'emit({"steam": OS.has_feature("steam"), "editor": OS.has_feature("editor"),
...
swallowtail export "Android Play" --out out/game.aab
```
...
```
swallowtail android list-devices                       # serial + state per attached device
swallowtail android preset-info                        # index, name, export_path, package_name
swallowtail android deploy --preset-name "Android Dev" --device-serial R5CT10 --debug=true
swallowtail android deploy --preset-name "Android Dev" --skip-export=true   # reinstall what exists
```
...
```powershell
swallowtail export "Android Dev" --pack --out out/receipt.pck
