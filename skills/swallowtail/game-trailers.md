...
records it, **ffmpeg** encodes it, and **swallowtail** drives the iteration loop between takes so a
...
```
swallowtail fs mkdir --path res://trailers
swallowtail scene create --path res://trailers/trailer.tscn --root-type Control
swallowtail scene open --path res://trailers/trailer.tscn
swallowtail script create --path res://trailers/trailer_director.gd --extends Control
swallowtail script attach --node-path . --script-path res://trailers/trailer_director.gd
```
...
```
swallowtail node add --type CanvasLayer --parent-path . --name CoverLayer
swallowtail node set --node-path CoverLayer --property layer --value 80
swallowtail node add --type ColorRect --parent-path CoverLayer --name Cover
swallowtail node set --node-path CoverLayer/Cover --property mouse_filter --value 2
```
...
```
swallowtail run res://trailers/trailer.tscn --write-movie out/trailer.avi --fixed-fps 60
```
...
`<project>/.godot/swallowtail-run.log`. Raise `--timeout` past the length of the cut: the default is
...
```
swallowtail project settings --section editor/movie_writer
...
```
swallowtail project settings --key display/window/size/viewport_width
swallowtail project set-setting --key display/window/size/viewport_width --value 1920
swallowtail project set-setting --key display/window/size/viewport_height --value 1080
```
...
```
swallowtail scene play --mode res://trailers/trailer.tscn
swallowtail runtime screenshot --save-path user://t.png       # repeat along the timeline
swallowtail runtime capture-frames --count 8 --frame-interval 30
```
...
```
swallowtail scene play
swallowtail runtime start-recording
# play the take by hand
swallowtail runtime stop-recording        # returns {events: [...], duration_ms: N}
```
