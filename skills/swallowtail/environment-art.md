...
The pass *after* the level is proven. `level-design.md` says "beautify last", and this is the
last: you run it only once the blockout, game feel, and encounters hold up. It's the **Art Pass** in
...
against the live engine (`engine class-info`), since shapes are durable but signatures evolve.
...
to a single `MeshInstance3D` (CSG re-evaluates its whole boolean tree every frame, so a static
mesh is far cheaper) with collision baked from the same shapes:
```
...
```
Then `scene save`. **Reuse** materials from a small `.tres` library rather than authoring a unique
material per node. Reach for a custom shader only when PBR can't express the surface.
...
the rest of the body, so it reads as a silent no-op unless you check `editor errors`. That is the
...
- **SDFGI** is dynamic, needs no bake, and suits large/outdoor scenes:
  `Environment.sdfgi_enabled = true`.
- **LightmapGI** is baked and cheapest at runtime, for static scenes: add a `LightmapGI` node, set
  static lights to bake, bake in-editor.
- **VoxelGI** sits between the two: dynamic-ish, mid cost, indoor.
- `ReflectionProbe` for local reflections; light groups / `light_energy` for control.

## WorldEnvironment & post, with restraint
...
projected AO, and puddles, all stamped onto existing geometry without new meshes. `FogVolume` for
localized fog/light shafts. A little of each sells the fantasy far past its cost.

**Particles interact with the world via dedicated nodes** (verified live): attractors
(`GPUParticlesAttractorSphere3D/Box/VectorField`, where `strength` pulls and negative pushes) and
colliders (`GPUParticlesCollisionBox3D/Sphere/SDF/HeightField`, where SDF bakes level geometry so
...
`GPUParticles3D`, then make `draw_pass_1` a trail mesh, either `RibbonTrailMesh` (flat streak:
...
taper along a `curve` (a width/radius profile you can ease to zero for comet tails). This is the
...
extrusion, `pixel_size` for world scale, `curve_step` for outline smoothness) gives real,
...
by `CanvasModulate`, so wrap it in a low-`layer` `CanvasLayer` to exempt it (see `lighting-2d.md`).
...
- `display/window/size/viewport_width/height` → a low **native resolution**: 640×360 reads
  SNES/Genesis-density, 320×180 reads NES-chunky. That is a *style* choice, not just technical;
  both divide 1080p/4K exactly. `window_width/height_override` sizes the desktop window
  independently, so you author low-res but display big.
- `display/window/stretch/mode` = `canvas_items` and `aspect` = `keep`, plus the load-bearing
  setting `stretch/scale_mode` = **`integer`**: whole-number multiples only, which is what
...
- **Camera-to-camera crossfades** without cutting: duplicate the target `Camera3D` into a
  ...
  `get_viewport().disable_3d = true` and the 3D world stops rendering for free.
...
- **Grayscale + 3-second tests still pass**, so beauty didn't kill legibility.
