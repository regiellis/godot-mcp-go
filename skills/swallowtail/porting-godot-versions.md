# Porting a Godot 4.x project to 4.7 with swallowtail
...
the game behaved before, and the same drive replayed after. `swallowtail` runs the game, drives it,
...
`swallowtail upgrade <phase>` runs the six phases below as commands. Each phase is a gate: it does
...
1. The launch log. `launch --headless` captures the editor's whole stdout and stderr to
   `.godot/swallowtail-launch.log`, unbounded, so boot-time failures (autoloads, `@tool` scripts)
...
error panel's may get worse. If either fails, `fix` restores the checkpoint and the captured files
named by the plan, and reports why with `debug state`'s stack attached. `--dry-run` prints the diff and touches
...
**Recovery captures the checkpoint and the plan's files before applying changes.** Each addon command commits its own
...
node identities and transforms; file snapshots hold the bytes and existence of each planned path.
The Git tag remains available for manual recovery. Automatic recovery leaves unrelated files alone,
refreshes cached scripts and project settings, and reloads affected scene tabs from their restored files.
Scene conversions require saved tabs and an editor that can report unsaved scenes. When a scene is
open, checkpoint capture must succeed before applying a fix; file-only fixes can run with no scene open. `restored: true` requires successful recovery calls, a clean
checkpoint diff, and matching file bytes after the editor refresh. Failures return `restored: false`
with per-step `recovery_errors`; a failed reload is never reported as restored. Two
...
Nothing here needs an editor, and `swallowtail install` does not dial one either. Launching the editor
is the step that starts rewriting the project, so it comes after all of this.

**Build:**
```
swallowtail install --project . --enable   # this dirties the tree, so it lands on its own
git commit -am "chore: install swallowtail addon"
swallowtail upgrade preflight --old-godot <old binary> --godot <new binary>
```
...
swallowtail check .                      # cold parse, no editor
```
...
**Build:**
```
swallowtail upgrade baseline --old-godot <old binary> --scenario drive.json
```

`upgrade baseline` runs the game standalone under the old binary and replays `drive.json` over the
game's own direct channel, which needs the `swallowtail/runtime/direct_server` project setting and a
...
planning around it: install it, enable the plugin, and run `swallowtail status` until the verdict
...
**Build:**
```
swallowtail upgrade open --godot <new binary>
```
...
Run by hand:
```
swallowtail status                  # closed or crashed -> launch one; running -> do not; starting -> wait
godot --path <project> --editor   # exactly one, at most one relaunch
swallowtail status                  # wait for running
...
**Build:**
```
swallowtail upgrade fix --category renames --dry-run
swallowtail upgrade fix --category renames --godot <new binary>
```
...
may get worse. If either fails, recovery restores and verifies the checkpoint and the plan's captured
files. The report carries `recovery_errors` if recovery is incomplete, plus the original failure and
the debugger's stack when available. A category that passes lands as one commit.
...
**Build:**
```
swallowtail upgrade fix --category tilemap --godot <new binary>
```
...
commits its own undo action and the wire cannot group several into one, so recovery uses the
checkpoint and file snapshots. Save scene tabs before running this category: recovery reloads
affected scenes from their captured disk state, then verifies the checkpoint diff.
...
**Build:**
```
swallowtail upgrade fix --category uid --godot <new binary>
```
...
**Build:**
```
swallowtail upgrade fix --category settings --godot <new binary>
```
...
**Build:**
```
swallowtail upgrade verify --godot <new binary>
```
...
- Exactly one editor launched, `swallowtail status` consulted first.
