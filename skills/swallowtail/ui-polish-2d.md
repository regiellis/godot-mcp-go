...
fighting you. Distilled from a shipped-quality game built *with swallowtail itself* against
...
truth for the look**, and never hard-code a color or size anywhere else. Discipline that pays:
...
- **Deterministic scruff**: hand-authored tilt lists cycled by index (`TILTS[i % n]`), never
  runtime `randf`, which re-rolls on every redraw and jitters.
...
- **`Label.label_settings` disables the theme for that label entirely**, so `add_theme_*_override`
...
- **Shared resources are shared**: a `LabelSettings`/`FontVariation` preloaded by N instances is
  one object, and the last writer styles them all. `duplicate()` a private copy per instance,
...
- **Letter-spacing** = a `FontVariation` with `set_spacing(TextServer.SPACING_GLYPH, em * size)`,
  cached per (em, size). That is real metrics, not spaces stuffed between characters.
...
- Cache `StyleBoxFlat`s by (color, radius), because allocation per redraw churns; the palette
  bounds the set.
...
- A ring/outline = a larger rounded rect drawn *behind* the face (`rect.grow(w)`, radius + w),
  which is CSS `box-shadow: 0 0 0 4px` spread. Godot has no dashed border either: stroke the
...
- **Game pieces resolve their own colors**. A `Die`/card exposes rule state (`role`, `selected`,
  ...
  pieces, because per-screen tinting drifts. Bonus: a concealed piece checks *concealment
  first*, so a bluff can't leak through the fill.
...
a few px of camera `offset` lean toward the actor, felt rather than seen. Two live-verified
camera caveats. `Camera2D.zoom` above 1.0 pushes IN, so never go below 1.0 if your board is
drawn viewport-sized (the void shows). And a second tween on the same property **cancels the
one already running**, so "punch then settle" must wait out the hit before issuing the settle,
or the pair collapses to a no-op.
...
builder has done its job. The traps, each one a silent failure and all hit live:
...
- **Minimum-size caching outside the tree**: `get_combined_minimum_size()` refreshes on a
  deferred update that never runs for an out-of-tree builder, so writing `size` clamps to whatever
