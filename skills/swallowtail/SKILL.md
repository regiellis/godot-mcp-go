---
name: swallowtail
description: Use the complete Godot development loop from the swallowtail CLI: discover the live engine API, build and edit in the open editor, run and play the game, observe state, debug failures, fix them in place, and verify the result. Use when the task involves creating or modifying a Godot project, testing game behavior, diagnosing a running game, or answering "does Godot/this node support X" against the actual engine.
---
...
2. **The CLI binary.** In this repo: `task build` → `bin/swallowtail.exe`. Invoke as `bin/swallowtail.exe <group> <command> [--flags]` (or `task run -- <group> <command> [--flags]`). Elsewhere, use the installed `swallowtail`.
...
swallowtail engine class-info --class Control --filter transform   # a class's own members + signatures
swallowtail engine classes --inherits Node2D            # what subclasses exist
swallowtail engine defaults --class OmniLight3D          # a class's default property values (no instantiation)
swallowtail engine version                              # confirm the running version
swallowtail engine commands [--group node]              # the MCP's own tool surface, with param docs per command
swallowtail node set --help                             # a command's param table (name/type/required/desc)
swallowtail engine doc-search --query "word wrap"       # search the docs PROSE by concept (finds the AUTOWRAP_* enum)
swallowtail engine class-info --class Label --filter autowrap   # a property ON a class: search the class, not the prose
swallowtail engine docs --class Label --member autowrap_mode   # what a member MEANS, from this build's own docs
```
...
- `swallowtail [--port N] [--timeout 30s] <group> <command> [--key value ...]`. Global flags go **before** the group.
...
`launch` is not the only subcommand that runs the engine binary directly. These five start a fresh Godot process and never dial the addon, so they work with the editor closed. Each writes the engine's output to `<project>/.godot/swallowtail-<name>.log` and returns the exact argv it ran, so you can reproduce a failure by hand.

- **`swallowtail check <file|dir>... [--jobs 4]`** parses `.gd` files cold and exits 1 if any fails, reporting `file:line` per error. Directories are walked recursively, skipping `.godot/` and `addons/` unless the path you name is itself inside addons. This is a parse check with no editor behind it: for the in-editor form that sees the open project, use `script validate`.
- **`swallowtail test [dir|file]...`** runs pure-logic tests cold: no editor, no scene tree, and the project's autoloads are never started. With no path it takes `res://test/`, swept recursively for `*_test.gd` and `test_*.gd`. A test file is any script that extends `RefCounted`; every `test_*` method is a test and takes one argument, a harness with `t.ok(cond, msg)`, `t.eq(actual, expected, msg)`, `t.ne(actual, other, msg)` and `t.fail(msg)`. The harness keeps going after a failure, so one method reports all of them. A test also fails when the engine prints a script error inside it, and a file that does not parse is reported as one failed file with the parse message. Unlike `import` it does not refuse while an editor is open. This is for save migrations, math, serializers and other logic that needs no tree; for anything that needs the running game, use the addon's `test run-scenario` and the `runtime`/`input` groups.
- **`swallowtail import`** runs the asset import a fresh clone or a CI checkout needs before anything else will open. It **refuses while an editor is running** on the project, because both own the same `.godot/` cache. To reimport one file in a live editor, use `import reimport --path res://...` instead.
- **`swallowtail run [scene] [--headless] [--debug-collisions] [--write-movie out.avi]`** starts the game standalone. With the `swallowtail/runtime/direct_server` project setting on it waits for the game's own channel and reports the port, which you then drive with `swallowtail --game runtime tree`. It refuses to start a second game while one is live, the same no-stacking rule `launch` applies to editors. `--write-movie` ends when the scene quits, so the command waits for the process and reports whether the film landed.
- **`swallowtail export <preset> [--debug|--pack] [--out PATH]`** runs the headless export and reports the file produced, its size, and the engine's error lines. An export that exits 0 having written nothing exits non-zero here, which is what a missing export template looks like. The parsed errors name each missing file.
...
- `scene2d add_sprite|add_camera|add_body`: 2D scene assembly (the canvas counterpart to `scene3d`). `add_body` wires a physics body + `CollisionShape2D` + shape (rectangle/circle/capsule) in **one** call. Its 3D twin is `scene3d add_body` (box/sphere/capsule primitives, or a trimesh/convex collider built from a MeshInstance3D via `--from-mesh`, which covers the "make this imported geometry collidable" need). With `--from-mesh` the body is seated on the mesh, its scale is baked into the shape, a primitive is sized to the mesh bounds, and `--reparent-mesh true` moves the mesh under the body so the two move together. Read the collider back with `node get` on the CollisionShape3D rather than `spatial bounds`, which reports visual geometry only.
...
swallowtail node get --node-path <path>        # inspect a node's properties
```

### Build a 2D scene
```
swallowtail scene create --path res://scenes/player.tscn --root-type CharacterBody2D   # opens it; --open=false to just write the file
swallowtail node add --type Sprite2D --name Sprite --parent-path .
swallowtail node add --type CollisionShape2D --name Col --parent-path .   # root is already the body; just add the shape
swallowtail node add-resource --node-path Col --property shape --resource-type RectangleShape2D
swallowtail script create --path res://scenes/player.gd --extends CharacterBody2D
swallowtail script attach --node-path . --script-path res://scenes/player.gd
swallowtail scene save

# Dropping a *separate* body into a level (platform, wall, trigger)? One call does body+shape:
swallowtail scene2d add-body --type StaticBody2D --shape rectangle --size "Vector2(256,32)" --name Platform --position "Vector2(0,400)"
```
...
swallowtail runtime tree                                  # live scene tree
...
swallowtail runtime capture-frames --count 5 --frame-interval 6   # observe motion
...
```
swallowtail test run-scenario --steps '[
