# Mobile & touch input: building with swallowtail

Touch, multitouch, gestures, on-screen controls, and safe areas the Godot way. Every class and
setting below was introspected against the live build, but **re-verify with `engine class-info`
...
Verified: `ScreenTouch` has `index`, `position`, `pressed`, `canceled` (OS stole the touch, so
treat it as a lift), `double_tap`; `ScreenDrag` adds `relative` (delta since last event) and
`velocity`.
...
- **`TouchScreenButton`** is a 2D node (not a `Control`) that fires an InputMap **action** from a
...
- **`VirtualJoystick`** is a `Control` node **in the running build's ClassDB** (engine-level, newer
  than most training data; builds can differ, so confirm with
  ...
  existing `Input.get_vector(...)` movement code works unchanged, with no glue. Verified members
  and defaults:
  - `action_left`/`action_right`/`action_up`/`action_down` (StringName, default
    `ui_left`…`ui_down`), which you point at your own actions; `joystick_mode`/`visibility_mode`
    (int enums, default 0, where the touchscreen-only value hides it on desktop); feel via
    `joystick_size` (100), `tip_size` (50), `deadzone_ratio` (0), `clampzone_ratio` (1).
  - Signals `pressed`, `tapped`, `released(input_vector)`, `flicked(input_vector)`, and
...
`DisplayServer.get_display_safe_area()` returns a `Rect2i` (screen pixels) covering the region
clear of notches, punch-holes, and rounded corners (verified; method exists). Inset a full-rect HUD from
...
Re-applying on `size_changed` matters, because orientation flips move the safe rect.
...
keybinds); `InputEventMIDI` carries note/velocity/channel for MIDI controllers, worth reaching
for only in instrument/rhythm games (see `rhythm-games.md`).
