...
> ```
> swallowtail authoring checkpoint --action capture --label pre-cleanup
> # …run the cleanup commands…
> swallowtail authoring checkpoint --action diff      # see exactly what moved
> ```
...
2. **Lights** come second, because Unreal's physical light units arrive as garbage magnitudes.
3. **Junk geometry**: editor-only meshes that clutter the tree and the viewport.
4. **Imports** stay invisible until you try to reimport or move a file, so do them last.
...
The Environment is a **sub-resource of the tiny `…WorldEnvironment.tscn`** the exporter emits, so open *that* scene (not the big level) and fix it there:

```
swallowtail scene open --path res://…/UnrealGodotWorldEnvironment.tscn
swallowtail cleanup unreal-env --tonemap agx --background_intensity 2.0 \
    --ssr=false --ssao=false --sdfgi=false --clear_auto_exposure
swallowtail scene save
```

- `--tonemap agx` picks AgX, which matches UE5's modern filmic look far better than Filmic. (`linear|reinhard|filmic|aces|agx` → 0..4.)
- `--background_intensity` drops the blown sky. Godot's *own* default is `30000`, so the exported `9716` isn't absurd in isolation; the problem is it driving ambient. Start low (1 to 5) and raise if the sky reads too dark.
...
- Effect toggles are **only applied when you pass them**, so omitting `--glow` leaves glow as-is.

It reports `before`/`after` for every field it touched. Verify numerically, and only then take **one** screenshot.

---

## 2. The lighting is too bright: `cleanup.unreal_lights`

**The trap (a real Godot API gotcha).** `light_intensity_lumens` and `light_intensity_lux` are **two inspector aliases for one stored intensity param**. The inspector shows "lux" on a `DirectionalLight3D` and "lumens" on omni/spot, but it's the same number underneath. So when the exporter dumps a UE intensity like `187500000` onto a directional's "lumens", Godot reads it **as lux** and the sun nukes the scene. It is *not* inert.

The magnitudes are garbage, and there's no clean unit back-conversion. So don't try to recover them; **normalize**:

```
swallowtail cleanup unreal-lights --dry-run     # see the plan: intensity_before/after, energy
swallowtail cleanup unreal-lights               # apply
swallowtail scene save
```

What it does, per light:
- Resets the intensity store to Godot's daylight/point default (**directional 100000, omni/spot 1000**), which is sane in either unit mode.
...
```
swallowtail scene open --path res://…/UnrealGodottestmap.tscn   # the level itself
swallowtail cleanup strip-junk --dry-run        # list matches + the reason each matched
swallowtail cleanup strip-junk                  # delete (undoable)
swallowtail scene save
```
...
- **Decals are kept by default**. A real export can have *thousands* of decal references (this one had ~1760) and many are real surface detail, so stripping them is a judgement call. Add `--include-decals` only when you've decided they're UI/debug junk.
...
The real bug is **paths**: every `.import` says `source_file="res://RailBridge/…"` because the exporter assumes the export sits at the **project root**. Drop it into a subfolder (`res://scenes/UnrealGodot…`) and that source path no longer exists. Rendering still works (uid), but **reimport-from-source is broken**.

```
swallowtail cleanup fix-imports --path res://scenes --dry-run   # list source_file mismatches
swallowtail cleanup fix-imports --path res://scenes --reimport  # rewrite + reimport
```

- For each `*.import` it compares `source_file` to the actual sibling asset and rewrites the mismatched line (a **single-line text replace**, so every other import param is preserved), but **only when the real sibling file exists** (the safe case; `actual_exists` in the report).
...
swallowtail scene open --path res://scenes/UnrealGodotWorldEnvironment.tscn
swallowtail cleanup unreal-env --tonemap agx --background_intensity 2.0 --ssr=false --sdfgi=false --clear_auto_exposure
swallowtail scene save

# 2. lights + 3. junk (in the level)
swallowtail scene open --path res://scenes/UnrealGodottestmap.tscn
swallowtail cleanup unreal-lights
swallowtail cleanup strip-junk
swallowtail scene save

# 4. imports (project-wide)
swallowtail cleanup fix-imports --path res://scenes --reimport

swallowtail authoring checkpoint --action diff
```
