...
```sh
swallowtail script create --path res://waves/event_schedule.gd --content "..."
swallowtail scene open --path res://arena.tscn
swallowtail node add --type Node --name Schedule --parent-path .
swallowtail script attach --node-path Schedule --script-path res://waves/event_schedule.gd
swallowtail script attach --node-path . --script-path res://waves/arena.gd   # builds the table
swallowtail scene save && swallowtail editor reload
swallowtail script validate --all
...
```sh
swallowtail runtime eval --code 'emit({"grunts": get_tree().get_nodes_in_group("grunt").size(),
...
```sh
swallowtail runtime eval --code 'var s = get_tree().current_scene.get_node("Schedule")
...
for i in 20:
	s.report_kill()
emit("reported")'
swallowtail test run-scenario --steps '[{"type":"wait","seconds":2.2}]'
swallowtail runtime get --node-path Schedule --properties '["kill_rate"]'
# "kill_rate": 3.33
```
