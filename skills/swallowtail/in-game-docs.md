...
```
swallowtail doc gym --gaps "[1.5,2.5,3.5,4.5]" --heights "[0.4,0.8,1.2]" --slopes "[25,40,55]" --spacing 4
```
...
```
swallowtail doc metric --type gap     --value 3.5 --at "Vector3(0,0,0)"  --difficulty hard
swallowtail doc metric --type height  --value 1.2 --at "Vector3(0,0,4)"  --difficulty easy
swallowtail doc metric --type slope   --value 35  --at "Vector3(0,0,8)"
swallowtail doc metric --type distance --value 5  --at "Vector3(0,0,12)"   # a labeled measuring stick
```
...
```
swallowtail doc zoo --from res://assets/props --cols 6
...
```
swallowtail doc zoo --scenes '["res://enemies/grunt.tscn","res://enemies/brute.tscn"]' --scale-ref --lighting=false
```
...
```
swallowtail doc museum --exhibits '["Cloth", {"name":"Destruction","link":"https://docs.godotengine.org/…","text":"how it shatters"}, {"name":"Scripting","text":"live cat-script demo"}]'
...
swallowtail doc note --action add --at "Vector3(40,0,12)" --category todo \
...
swallowtail doc note --action add --node-path selected --category bug --text "boss name changed 3 times, pick one"

# review the open notes (filter by category; resolved are hidden by default)
swallowtail doc note --action list
swallowtail doc note --action list --category art --include-resolved

# close one out (or --delete to remove the marker entirely)
swallowtail doc note --action resolve --node-path "Note_Todo"
```
...
- **Spatial notes** are `Marker2D` plus `node set-meta`. Metadata rides in the scene exactly as `_doc_note` does on a 3D marker, so the note stays attached to what it describes. Read one back with `swallowtail node get-meta --node-path X --key _doc_note`, or the whole set with `swallowtail doc note --action list`, which walks a 2D scene and reports each note with a path that feeds straight back into other commands.

**Build a 2D gym**: three ground slabs separated by two measured gaps, each graded and labeled:

```
swallowtail scene create --path res://docs/gym_2d.tscn --root-type Node2D --root-name Gym2D

# ground the controller runs along; the gaps between slabs are what's under test
swallowtail scene2d add-body --type StaticBody2D --name Ground_0 --shape rectangle --size "Vector2(256,32)" --position "Vector2(0,400)"
swallowtail scene2d add-body --type StaticBody2D --name Ground_1 --shape rectangle --size "Vector2(256,32)" --position "Vector2(400,400)"
swallowtail scene2d add-body --type StaticBody2D --name Ground_2 --shape rectangle --size "Vector2(256,32)" --position "Vector2(864,400)"
...
swallowtail node add --type ColorRect --name Grade_144 --properties '{"position":"Vector2(128,432)","size":"Vector2(144,16)","color":"#3faf5a"}'
swallowtail node add --type ColorRect --name Grade_208 --properties '{"position":"Vector2(528,432)","size":"Vector2(208,16)","color":"#d86b2b"}'

# the measurement, readable in the viewport
swallowtail node add --type Label --name Gap_144 --properties '{"position":"Vector2(140,340)","text":"144 px (easy)"}'
swallowtail node add --type Label --name Gap_208 --properties '{"position":"Vector2(540,340)","text":"208 px (max jump)"}'

# a spatial note carrying its own to-do
swallowtail node add --type Marker2D --name Note_Tuning --properties '{"position":"Vector2(632,300)"}'
swallowtail node set-meta --node-path Note_Tuning --key _doc_note \
...
```sh
swallowtail script create --path res://debug/debug_overlay.gd --content "..."
swallowtail scene play --mode main

# what is registered, and proof nothing draws while every layer is off
swallowtail runtime eval --code 'var o = get_tree().current_scene.overlay
...
swallowtail runtime call --node-path DebugOverlay --method toggle --args '["attack_range", true]'
swallowtail runtime call --node-path DebugOverlay --method toggle --args '["spawn_bounds", true]'
swallowtail runtime call --node-path DebugOverlay --method toggle --args '["viewport_bounds", true]'
