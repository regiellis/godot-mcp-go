...
black erases everything). There is one per canvas, and `lighting canvas-modulate
--color '#26264d'` creates or updates it. Tint it for mood as well as night: a dusk orange or cave blue is the
...
additive blending (it brightens what is behind it, which is right for energy and heat). `--mode
light_only` is the inverse: visible *only* where light hits. That is the tool for glow-decal layers
