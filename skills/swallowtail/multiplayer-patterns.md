  Godot high-level multiplayer patterns with swallowtail
...
- **One `MultiplayerAPI` per node**. Each `Node` exposes `multiplayer` (verified), and by
  default all share the tree's one API. It carries the peer, the connection signals, and
...
- **Server-authoritative is the default**. State lives on the server; clients send **intent**
  ...
  truth, which is what makes a game hard to cheat. The pragmatic co-op shortcut (client owns
...
- **Server owns the world**, meaning enemies, doors, score, and physics. Authority stays `1`.
  Discrete facts go out as RPCs from the server; continuous state rides a synchronizer.
...
- **Who may call:** `"authority"` (the default, only the node's authority) or `"any_peer"`
  (required for client→server intent).
- **Local execution:** `"call_remote"` (the default, remote peers only) or `"call_local"` (also
  runs on the caller, so the sender sees its own effect).
- **Transfer mode:** `"reliable"` (resent until acked, for state and chat), `"unreliable"`
  (the default, for high-frequency streams), `"unreliable_ordered"` (drops stale, keeps order);
  plus a trailing **int** channel (default `0`) to avoid head-of-line blocking between traffic.
...
```
   ...
   MultiplayerSpawner replicates node existence
...
- **Auto-spawn:** when the **server** `add_child()`s a node under the spawner's `spawn_path`
  ...
  That is the common path, and you never call `spawn()` yourself.
...
```
   ...
   MultiplayerSynchronizer replicates node properties
...
- **`sync` vs `watch`:** a *synced* property sends every interval; a *watched* one sends (on
  `delta_interval`) only when it changes, which is cheaper for rarely-moving state. *spawn*
  includes the value in the initial spawn snapshot.
- **`replication_interval`** throttles frequency, and `0` (default) = every network frame (the
...
**2. Game scene** holds a `Players` container plus the spawner over it:
...
- **Continuous state → `MultiplayerSynchronizer`**. Positions, rotations, and health change
  every frame, and only the *latest* value matters (a dropped unreliable packet self-corrects).
- **Discrete events → RPCs**. Fire, jump, pickup, chat, and "round over" are one-shot facts
  that must arrive exactly once (`"reliable"`) and are meaningless to poll.
