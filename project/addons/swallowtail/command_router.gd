...
const _BUILTIN_GROUPS: Array = [
	"res://addons/swallowtail/commands/project_commands.gd",
	"res://addons/swallowtail/commands/scene_commands.gd",
	"res://addons/swallowtail/commands/node_commands.gd",
	"res://addons/swallowtail/commands/spatial_commands.gd",
	"res://addons/swallowtail/commands/authoring_commands.gd",
	"res://addons/swallowtail/commands/script_commands.gd",
	"res://addons/swallowtail/commands/csharp_commands.gd",
	"res://addons/swallowtail/commands/editor_commands.gd",
	"res://addons/swallowtail/commands/debug_commands.gd",
	...
	"res://addons/swallowtail/commands/theme_commands.gd",
	"res://addons/swallowtail/commands/shader_commands.gd",
	"res://addons/swallowtail/commands/particle_commands.gd",
	"res://addons/swallowtail/commands/scene_3d_commands.gd",
	"res://addons/swallowtail/commands/scene_2d_commands.gd",
	"res://addons/swallowtail/commands/material_commands.gd",
	"res://addons/swallowtail/commands/csg_commands.gd",
	"res://addons/swallowtail/commands/gridmap_commands.gd",
	"res://addons/swallowtail/commands/scatter_commands.gd",
	"res://addons/swallowtail/commands/lighting_commands.gd",
	"res://addons/swallowtail/commands/path_commands.gd",
	"res://addons/swallowtail/commands/pcg_commands.gd",
	"res://addons/swallowtail/commands/wfc_commands.gd",
	"res://addons/swallowtail/commands/mesh_commands.gd",
	"res://addons/swallowtail/commands/doc_commands.gd",
	"res://addons/swallowtail/commands/cleanup_commands.gd",
	"res://addons/swallowtail/commands/physics_commands.gd",
	"res://addons/swallowtail/commands/navigation_commands.gd",
	"res://addons/swallowtail/commands/audio_commands.gd",
	"res://addons/swallowtail/commands/input_map_commands.gd",
	"res://addons/swallowtail/commands/resource_commands.gd",
	"res://addons/swallowtail/commands/fs_commands.gd",
	"res://addons/swallowtail/commands/import_commands.gd",
	"res://addons/swallowtail/commands/multiplayer_commands.gd",
	"res://addons/swallowtail/commands/skeleton_commands.gd",
	"res://addons/swallowtail/commands/localization_commands.gd",
	"res://addons/swallowtail/commands/ui_commands.gd",
	"res://addons/swallowtail/commands/camera_commands.gd",
	"res://addons/swallowtail/commands/analysis_commands.gd",
	"res://addons/swallowtail/commands/batch_commands.gd",
	"res://addons/swallowtail/commands/profiling_commands.gd",
	"res://addons/swallowtail/commands/export_commands.gd",
	"res://addons/swallowtail/commands/test_commands.gd",
	"res://addons/swallowtail/commands/android_commands.gd",
	"res://addons/swallowtail/commands/stats_commands.gd",
]
...
func execute(method: String, params: Dictionary, operation: Dictionary = {}) -> Dictionary:
	...
	if not _handlers.has(method):
		...
		if not rejection.is_empty():
			...
			if method in ["test.run_scenario", "script.validate", "import.reimport"]:
				result = await _handlers[method].call(params, operation)
			else:
				result = await _handlers[method].call(params)
	# Don't record the dashboard's own stats polling.
	if not method.begins_with("stats."):
		_record(method, not result.has("error"), Time.get_ticks_msec() - t0, params, result.get("error", {}))
	return result
...
func _record(method: String, ok: bool, ms: int, params: Dictionary, failure: Dictionary = {}) -> void:
	_total += 1
	if not ok:
		_errors += 1
	...
	var safe_params: Dictionary = _diagnostic_value(params)
	var params_json := JSON.stringify(safe_params)
	var replayable := params_json.to_utf8_buffer().size() <= 16384 and safe_params == params
	var entry := {
		"ts": int(Time.get_unix_time_from_system() * 1000.0),
		"method": method,
		"ok": ok,
		"ms": ms,
		"params": _summarize(safe_params),
		"replay_available": replayable,
	}
	if replayable:
		entry["request_params"] = safe_params
	if not failure.is_empty():
		var safe_error: Dictionary = _diagnostic_value(failure)
		if JSON.stringify(safe_error).to_utf8_buffer().size() > 16384:
			safe_error = {"code": failure.get("code", -32603), "message": String(safe_error.get("message", "Error")).left(1024), "data_truncated": true}
...
## Bound retained diagnostics and omit common credential fields. A changed or
## oversized request is never offered as a replayable call.
func _diagnostic_value(value: Variant, depth: int = 0) -> Variant:
	if depth > 12:
		return "[depth limit]"
	if value is Dictionary:
		var clean := {}
		for key in value:
			var name := String(key).to_lower().replace("-", "_")
			if "password" in name or "secret" in name or "token" in name or name in ["authorization", "api_key", "apikey", "cookie"]:
				clean[key] = "[redacted]"
			else:
				clean[key] = _diagnostic_value(value[key], depth + 1)
		return clean
	if value is Array:
		var clean: Array = []
		for item in value.slice(0, 1024):
			clean.append(_diagnostic_value(item, depth + 1))
		if value.size() > 1024:
			clean.append("[items truncated]")
		return clean
	if value is String and value.length() > 16384:
		return value.left(16384) + "[truncated]"
	return value
