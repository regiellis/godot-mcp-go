...
## Port precedence: SWALLOWTAIL_GAME_PORT env pins a single port (wins); else scan
## the 9200-9215 range so multiple game instances each grab a free one.
func _candidate_ports() -> Array:
	var env := Identity.env("SWALLOWTAIL_GAME_PORT")
...
func _enqueue_runtime(ws: WebSocketPeer, id: Variant, method: String, params: Dictionary) -> void:
	...
	if method == "runtime.eval":
		...
		print("[MCP Game] runtime.eval executing (%d bytes):\n%s\n[MCP Game] --- end runtime.eval ---" % [code.length(), code])
