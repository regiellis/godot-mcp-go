...
func _validate(params: Dictionary) -> Dictionary:
	if params.has("path"):
		var requested := require_string(params, "path")
		if requested[1] != null:
			return requested[1]
		return _validate_saved_scene(normalize_project_path(requested[0]))
	var root := get_edited_root()
	if root == null:
		return error_no_scene()
	var issues: Array = []
...
## Inspect serialized SceneState, including inherited and instanced scenes.
## No Node is instantiated, so @tool node constructors cannot run and editor
## selection, tabs, and unsaved changes remain untouched.
func _validate_saved_scene(path: String) -> Dictionary:
	if not path.begins_with("res://") or path.contains(".."):
		return error_invalid_params("scene.validate path must be inside res://")
	if path.get_extension().to_lower() not in ["tscn", "scn"]:
		return error_invalid_params("scene.validate path must name a .tscn or .scn file")
	if not FileAccess.file_exists(path):
		return error_not_found("Scene file '%s'" % path)
	var issues := _missing_ext_resources(path)
	if issues.is_empty():
		var cache_mode := ResourceLoader.CACHE_MODE_IGNORE
		if ClassDB.class_has_integer_constant("ResourceLoader", "CACHE_MODE_IGNORE_DEEP"):
			cache_mode = ClassDB.class_get_integer_constant("ResourceLoader", "CACHE_MODE_IGNORE_DEEP")
		var packed := ResourceLoader.load(path, "PackedScene", cache_mode) as PackedScene
		if packed == null:
			issues.append({"type": "scene_load_error", "file": path, "detail": "Saved scene could not be loaded"})
		else:
			var nodes: Dictionary = {}
			_collect_saved_nodes(packed.get_state(), ".", nodes, issues, [])
			for node_path: String in nodes:
				var entry: Dictionary = nodes[node_path]
				var props: Dictionary = entry["properties"]
				for prop: String in props:
					var value: Variant = props[prop]
					if value is NodePath and not value.is_empty():
						var target := _saved_node_target(node_path, value)
						if target != "" and not nodes.has(target):
							issues.append({"type": "unresolved_node_path", "node": node_path, "property": prop, "path": String(value)})
				if entry["type"] == "AnimationPlayer":
					var anim_root := _saved_node_target(node_path, props.get("root_node", NodePath("..")))
					if anim_root == "":
						continue
					var libraries: Dictionary = props.get("libraries", {})
					for lib_name: String in libraries:
						var lib := libraries[lib_name] as AnimationLibrary
						if lib == null:
							continue
						for anim_name in lib.get_animation_list():
							...
							for track in anim.get_track_count():
								...
								var target := _saved_node_target(anim_root, track_path)
								if target != "" and not nodes.has(target):
									issues.append({"type": "missing_animation_track", "node": node_path, "animation": String(anim_name) if lib_name == "" else lib_name + "/" + anim_name, "track": track, "track_path": String(track_path)})
	return success({"scene_path": path, "source": "disk", "valid": issues.is_empty(), "issue_count": issues.size(), "issues": issues,
		"note": "Checks saved node paths, classes, animation tracks and external resources without opening the scene. Absolute paths depend on the runtime tree and are not checked; use script.validate for script diagnostics."})


func _saved_node_target(base: String, path: NodePath) -> String:
	if path.is_absolute():
		return ""
	var names := String(path.get_concatenated_names())
	return (base + "/" + names).simplify_path().trim_prefix("./") if names != "" else base


func _collect_saved_nodes(state: SceneState, prefix: String, nodes: Dictionary, issues: Array, ancestors: Array) -> void:
	if state == null:
		return
	if ancestors.has(state) or ancestors.size() >= 64:
		issues.append({"type": "scene_recursion", "node": prefix, "detail": "Scene nesting is cyclic or exceeds 64 levels"})
		return
	var chain := ancestors.duplicate()
	chain.append(state)
	_collect_saved_nodes(state.get_base_scene_state(), prefix, nodes, issues, chain)
	for i in state.get_node_count():
		var node_path := _saved_node_target(prefix, state.get_node_path(i))
		var instance := state.get_node_instance(i)
		if instance != null:
			_collect_saved_nodes(instance.get_state(), node_path, nodes, issues, chain)
		var node_type := String(state.get_node_type(i))
		var entry: Dictionary = nodes.get(node_path, {"type": node_type, "properties": {}})
		if node_type != "":
			entry["type"] = node_type
			if not ClassDB.class_exists(node_type):
				issues.append({"type": "missing_node", "node": node_path, "original_class": node_type})
		for p in state.get_node_property_count(i):
			entry["properties"][String(state.get_node_property_name(i, p))] = state.get_node_property_value(i, p)
...
func get_command_docs() -> Dictionary:
	return {
		...
		"scene.validate": {
			"description": "Validate the active scene, or a saved scene by path without switching tabs or changing unsaved work. Checks node paths, animation tracks, missing classes and external resources. Saved mode inspects serialized state without instantiating nodes; absolute runtime paths and script diagnostics are outside its scope.",
			"params": [doc_param("path", "String", false, "Saved .tscn/.scn under res:// to inspect from disk. Omit to validate the active scene including unsaved changes.")],
		},
	}
