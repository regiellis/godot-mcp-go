...
func _reimport(params: Dictionary, operation: Dictionary = {}) -> Dictionary:
	var paths: Array = []
	if params.has("paths"):
	...
	for path in valid:
		...
		if operation_cancelled(operation):
			return error(-32800, "Import cancelled between assets", {"reimported": completed})
		# Godot's native import of one asset cannot be interrupted safely.
		efs.reimport_files(PackedStringArray([path]))
		completed.append(path)
		operation_progress(operation, completed.size(), valid.size(), "Reimported %s" % path)
