...
## <project>/.godot/swallowtail.json keeps an accurate http_port.
...
const MAX_BUFFER := MAX_BODY + MAX_HEADERS   # includes queued bytes while a command awaits
...
const MCP_PATH := "/mcp"
const PORT_SETTING := "swallowtail/network/http_port"
const TYPED_SETTING := "swallowtail/network/http_typed"
...
## the client at initialize (MCP InitializeResult.instructions). The swallowtail skill
## carries the detail; this is the durable minimum that rides along without it.
const INSTRUCTIONS := "This endpoint drives a running Godot editor (4.7+) through the swallowtail addon. " + \
...
## Port precedence, mirroring the WebSocket server: SWALLOWTAIL_HTTP_PORT env pins a
## single port (wins); else the per-project setting swallowtail/network/http_port if
## > 0; else scan the auto range so instances each grab a free one.
func _candidate_ports() -> Array:
	var env := Identity.env("SWALLOWTAIL_HTTP_PORT")
	if not env.is_empty() and env.is_valid_int():
		return [env.to_int()]
	var pinned := int(Identity.setting(PORT_SETTING, 0))
	if pinned > 0:
		return [pinned]
	var ports: Array = []
...
func _process(delta: float) -> void:
	...
	for conn in _conns:
		...
		if status == StreamPeerTCP.STATUS_CONNECTED:
			...
			if avail > MAX_BUFFER - (conn["buf"] as PackedByteArray).size():
				# Check before allocating, including while a handler is suspended.
				_send_status(peer, 413, "Payload Too Large", false)
...
func _initialize_result(params: Variant) -> Dictionary:
	...
	return {
		...
		"serverInfo": {"name": "swallowtail-addon", "version": _addon_version()},
		"instructions": INSTRUCTIONS,
	}


## tools/list: godot_run always first (the generic escape hatch, mirroring
## serve.go), then, unless swallowtail/network/http_typed is false, every typed
...
func _typed_enabled() -> bool:
	return bool(Identity.setting(TYPED_SETTING, true))
...
func _build_tool(tool_name: String, method: String, doc: Variant) -> Dictionary:
	...
	return {
		"name": tool_name,
		"description": "Swallowtail command '%s'. Parameters are addon-defined; discover them via engine_commands (group '%s') or the generic godot_run tool." % [method, method.get_slice(".", 0)],
...
func _schema_from_params(params: Variant) -> Dictionary:
	...
	if params is Array:
		for entry in (params as Array):
			...
			if pd.get("schema") is Dictionary:
				schema.merge(pd.schema, true)
			props[pname] = schema
...
func _addon_version() -> String:
	...
	if cfg.load("res://addons/swallowtail/plugin.cfg") == OK:
