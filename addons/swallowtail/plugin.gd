...
const Identity = preload("res://addons/swallowtail/identity.gd")

## Boots the MCP WebSocket server when the plugin is enabled and tears it down
## on exit. The addon is the SERVER; the swallowtail Go CLI is a client.
...
const _AUTOLOADS: Array = [
	["MCPGameInspector", "res://addons/swallowtail/services/game_inspector.gd"],
	["MCPGameInput", "res://addons/swallowtail/services/game_input.gd"],
]
...
const _PORT_SETTING := "swallowtail/network/port"
...
const _DIRECT_SERVER_SETTING := "swallowtail/runtime/direct_server"
...
const _MCP_HTTP_SETTING := "swallowtail/network/mcp_http"
const _HTTP_PORT_SETTING := "swallowtail/network/http_port"
const _HTTP_TYPED_SETTING := "swallowtail/network/http_typed"
...
func _enter_tree() -> void:
	...
	if enabled.has("res://addons/godot_mcp/plugin.cfg"):
		push_error("Swallowtail: legacy addon is still enabled. Close the editor and run swallowtail migrate before enabling Swallowtail.")
		return

	_register_settings()

	activity_log = preload("res://addons/swallowtail/services/activity_log.gd").new()
	...
	debugger_bridge = preload("res://addons/swallowtail/services/debugger_bridge.gd").new()
	add_debugger_plugin(debugger_bridge)
	debugger_bridge.ensure_connected()

	_router = preload("res://addons/swallowtail/command_router.gd").new()
	...
	# ("Swallowtail", set in the panel's _init, before this add). In-memory wiring like
	...
	stats_panel = preload("res://addons/swallowtail/services/stats_panel.gd").new()
	...
	if _http_enabled():
		_http_server = preload("res://addons/swallowtail/mcp_http_server.gd").new()
		_http_server.name = "MCPHttpServer"
		_http_server.command_router = _router
		add_child(_http_server)
		_http_server.start()

	_server = preload("res://addons/swallowtail/websocket_server.gd").new()
...
func _register_settings() -> void:
	# Seed renamed settings from persisted legacy values before registering defaults.
	for prop: Dictionary in ProjectSettings.get_property_list():
		var old_name := str(prop.get("name", ""))
		if old_name.begins_with("godot_mcp/"):
			var new_name := old_name.replace("godot_mcp/", "swallowtail/")
			if not ProjectSettings.has_setting(new_name):
				ProjectSettings.set_setting(new_name, ProjectSettings.get_setting(old_name))
...
func _http_enabled() -> bool:
	return bool(Identity.setting(_MCP_HTTP_SETTING, true))
