...
const LEGACY_DISCOVERY_PATH := "res://.godot/godot-mcp.json"
...
## Port precedence: SWALLOWTAIL_PORT env pins a single port (wins); else the
## per-project setting swallowtail/network/port if > 0 (how concurrent projects get
## deterministic ports); else scan the range so instances each grab a free one.
func _candidate_ports() -> Array:
	var env := Identity.env("SWALLOWTAIL_PORT")
	if not env.is_empty() and env.is_valid_int():
		return [env.to_int()]
	var pinned := int(Identity.setting("swallowtail/network/port", 0))
	if pinned > 0:
		return [pinned]
	var ports: Array = []
...
func _execute(ws: WebSocketPeer, id: Variant, method: String, params: Dictionary) -> void:
	...
	if ws.get_ready_state() != WebSocketPeer.STATE_OPEN:
		return
	var operation := {"cancelled": func() -> bool: return ws.get_ready_state() != WebSocketPeer.STATE_OPEN,
		"progress": func(done: int, total: int, message: String) -> void:
			if ws.get_ready_state() == WebSocketPeer.STATE_OPEN:
				ws.send_text(JSON.stringify({"jsonrpc": "2.0", "method": "godot/progress", "params": {"progress": done, "total": total, "message": message}}))}
	var result: Dictionary = await command_router.execute(method, params, operation)
...
func _write_discovery() -> void:
	...
	for path: String in [DISCOVERY_PATH, LEGACY_DISCOVERY_PATH]:
...
func _remove_discovery() -> void:
	for path: String in [DISCOVERY_PATH, LEGACY_DISCOVERY_PATH]:
		_remove_discovery_path(path)


func _remove_discovery_path(path: String) -> void:
...
func _clear_stale_discovery() -> void:
	for path: String in [DISCOVERY_PATH, LEGACY_DISCOVERY_PATH]:
		_clear_stale_discovery_path(path)


func _clear_stale_discovery_path(path: String) -> void:
