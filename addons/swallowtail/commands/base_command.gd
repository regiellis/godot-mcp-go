...
## Transport-owned callbacks, passed separately from untrusted command params.
func operation_cancelled(operation: Dictionary) -> bool:
	return operation.has("cancelled") and operation.cancelled.call()


func operation_progress(operation: Dictionary, done: int, total: int, message: String) -> void:
	if operation.has("progress") and not operation_cancelled(operation):
		operation.progress.call(done, total, message)


func operation_wait(operation: Dictionary, seconds: float) -> bool:
	var deadline := Time.get_ticks_msec() + int(seconds * 1000)
	while Time.get_ticks_msec() < deadline:
		if operation_cancelled(operation):
			return false
		await get_tree().process_frame
	return not operation_cancelled(operation)
