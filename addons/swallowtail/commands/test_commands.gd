...
func _run_scenario(params: Dictionary, operation: Dictionary = {}) -> Dictionary:
	...
	# Validate the entire plan before restarting a game or sending any input.
	for i in steps.size():
		var problems := _step_errors(steps[i])
		if not problems.is_empty():
			return error(-32602, "Invalid scenario step %d: %s" % [i, "; ".join(problems)], {"step": i, "problems": problems})
	...
	if not scene_path.is_empty():
		if EditorInterface.is_playing_scene():
			...
			if not await operation_wait(operation, 0.5):
				return error(-32800, "Scenario cancelled before playback")
		...
		if not await operation_wait(operation, 1.0):
			return error(-32800, "Scenario cancelled during playback startup")
	...
	for i in steps.size():
		if operation_cancelled(operation):
			return error(-32800, "Scenario cancelled", {"completed_steps": results.size()})
		...
		match step_type:
			"input":
				step_result.merge(await _execute_input_step(step, operation))
			"wait":
				step_result.merge(await _execute_wait_step(step, operation))
			"assert":
		...
		operation_progress(operation, i + 1, steps.size(), "Completed scenario step %d" % i)
...
## The published JSON Schema and preflight share the same variant definitions.
func _step_variants() -> Array:
	var text := {"type": "string", "minLength": 1}
	var boolean := {"type": "boolean"}
	var duration := {"type": "number", "minimum": 0}
	var frames := {"type": "integer", "minimum": 1}
	var variants: Array = []
	for shape in [
		["input", ["action"], {"action": text, "pressed": boolean, "strength": {"type": "number", "minimum": 0, "maximum": 1}, "auto_release": boolean, "frame_delay": frames, "consume_timeout": duration}],
		["input", ["keycode"], {"keycode": text, "pressed": boolean, "shift": boolean, "ctrl": boolean, "alt": boolean, "frame_delay": frames, "consume_timeout": duration}],
		["wait", [], {"seconds": duration}],
		["wait", ["node_path"], {"node_path": text, "timeout": duration, "poll_frames": frames}],
		["assert", ["text"], {"text": {"type": "string"}, "partial": boolean}],
		["assert", ["node_path", "property", "expected"], {"node_path": text, "property": text, "expected": {}, "operator": {"type": "string", "enum": ["eq", "neq", "gt", "lt", "gte", "lte", "contains", "type_is"]}}],
		["screenshot", [], {"save_path": text, "half_resolution": boolean}],
	]:
		var props: Dictionary = shape[2].duplicate(true)
		props["type"] = {"type": "string", "enum": [shape[0]]}
		var required: Array = ["type"]
		required.append_array(shape[1])
		variants.append({"type": "object", "properties": props, "required": required, "additionalProperties": false})
	return variants


func _step_errors(step: Variant) -> Array:
	if not step is Dictionary:
		return ["Expected an object"]
	var alternatives: Array = []
	for schema: Dictionary in _step_variants():
		if step.get("type") not in schema.properties.type.enum:
			continue
		var problems: Array = []
		...
		for key in step:
			if not schema.properties.has(key):
				...
				continue
			var rule: Dictionary = schema.properties[key]
			var value: Variant = step[key]
			var kind: String = rule.get("type", "")
			var matches := kind.is_empty() or (kind == "string" and value is String) or (kind == "boolean" and value is bool) or (kind in ["number", "integer"] and (value is int or value is float))
			if not matches:
				problems.append("'%s' must be %s" % [key, kind])
				continue
			if kind == "integer" and float(value) != floor(float(value)):
				problems.append("'%s' must be an integer" % key)
			if rule.has("minimum") and (not is_finite(float(value)) or value < rule.minimum):
				problems.append("'%s' must be >= %s" % [key, rule.minimum])
			if rule.has("maximum") and value > rule.maximum:
				problems.append("'%s' must be <= %s" % [key, rule.maximum])
			if rule.has("minLength") and value.length() < rule.minLength:
				problems.append("'%s' must not be empty" % key)
			if rule.has("enum") and value not in rule.enum:
				problems.append("'%s' must be one of %s" % [key, rule.enum])
		if problems.is_empty():
			if step.has("save_path"):
				var guard := guard_project_path(step.save_path)
				if not guard.is_empty():
					return [guard.error.message]
			return []
		if alternatives.is_empty() or problems.size() < alternatives.size():
			alternatives = problems
	return alternatives if not alternatives.is_empty() else ["Missing or unknown type; expected input, wait, assert, or screenshot"]
...
func _execute_input_step(step: Dictionary, operation: Dictionary = {}) -> Dictionary:
	var events: Array = []
	...
	var slot_was_clear := await _await_input_slot_clear(path, wait_sec, operation)
	if operation_cancelled(operation):
	...
	var consumed := await _await_input_slot_clear(path, wait_sec, operation)
...
func _await_input_slot_clear(path: String, timeout_sec: float, operation: Dictionary = {}) -> bool:
...
func _execute_wait_step(step: Dictionary, operation: Dictionary = {}) -> Dictionary:
	if step.has("node_path"):
		...
		if not await operation_wait(operation, seconds):
			return {"cancelled": true}
