...
const CSharpCommands := preload("res://addons/swallowtail/commands/csharp_commands.gd")
const GDScriptLinter := preload("res://addons/swallowtail/utils/gdscript_linter.gd")
...
func _validate(params: Dictionary, operation: Dictionary = {}) -> Dictionary:
	...
	if modified:
		return await _validate_modified(operation)
	if all:
		return await _validate_all(operation)
...
func _validate_one(path: String) -> Dictionary:
	...
	if file == null:
		return {
			...
			"diagnostics": [{"file": path, "line": 0, "severity": "error", "message": "Cannot read script."}],
	...
	# Preserve the source directory for relative preloads and the addons warning
	# exclusion, without occupying the real script's resource-cache slot.
	var compiled := _compile(_strip_class_name(source), path)
	var err: int = compiled.error

	if err == OK:
		return {"path": path, "valid": true, "message": "Script compiles successfully",
			"diagnostics": compiled.diagnostics, "diagnostics_available": compiled.diagnostics_available}
	return {
		"path": path,
		"valid": false,
		"error_code": err,
		...
		"diagnostics_available": compiled.diagnostics_available,
	}


## Run a list of res:// paths through _validate_one, returning only the failures.
func _validate_batch(paths: Array, mode: String, operation: Dictionary = {}) -> Dictionary:
	var checked := 0
	var passed := 0
	var failures: Array = []
	for res_path in paths:
		...
		if operation_cancelled(operation):
			return error(-32800, "Validation cancelled", {"checked": checked, "total": paths.size()})
		var result := _validate_one(res_path)
		checked += 1
		operation_progress(operation, checked, paths.size(), "Validated %s" % res_path)
...
func _validate_modified(operation: Dictionary = {}) -> Dictionary:
	...
	return await _validate_batch(paths, "modified", operation)
...
func _validate_all(operation: Dictionary = {}) -> Dictionary:
	var paths: Array = []
	_collect_all_gd("res://", paths)
	return await _validate_batch(paths, "all", operation)
...
func _compile(source: String, context_path: String = "") -> Dictionary:
	...
	if not context_path.is_empty():
		# A unique sibling path avoids both the real resource-cache slot and
		# temporary scripts retained after failed dependency compilation.
		script.resource_path = check_path
