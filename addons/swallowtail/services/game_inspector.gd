...
const DIRECT_SERVER_SETTING := "swallowtail/runtime/direct_server"
...
## AND the swallowtail/runtime/direct_server setting is on. The debug-build check is
...
func _maybe_start_direct_server() -> void:
	if not OS.is_debug_build():
		return
	if not bool(Identity.setting(DIRECT_SERVER_SETTING, false)):
		return
