...
## In-editor Swallowtail dashboard, docked on the right: the side-dock twin of the
## `swallowtail dashboard` web UI (both surfaces stay; this one costs no process
## and no port).
##
...
##
## Swallowtail's butler, mulberry accent, and speech bubbles share the docs site's
## identity. Data stays compact, with hairlines and mono numerals. Surfaces and
## readable accent variants follow the editor's light/dark theme.
##
...
const DISCOVERY_PATH := "res://.godot/swallowtail.json"
const PORT_RECHECK_MS := 10000

const BUTLER_NOTE := "res://addons/swallowtail/brand/note.webp"
const BUTLER_TIP := "res://addons/swallowtail/brand/tip.webp"
const BUTLER_WARNING := "res://addons/swallowtail/brand/warning.webp"
...
var _details_dialog: AcceptDialog
var _details_text: TextEdit
var _copy_plan_button: Button
var _selected_error: Dictionary = {}
var _export_dialog: FileDialog
var _export_snapshot: Dictionary = {}
...
var _feed_head: HBoxContainer
var _feed_empty: HBoxContainer
var _filter := "all"
...
var _sb_banner: StyleBoxFlat    # warning speech bubble
...
func _ready() -> void:
	...
	resized.connect(_update_feed_density)
	_update_feed_density()
...
func _build_header() -> void:
	...
	row.add_child(_make_mascot(BUTLER_NOTE, 40))
	# The name takes remaining width without imposing a minimum on the dock.
	var brand := Label.new()
	brand.text = "Swallowtail"
	brand.tooltip_text = "Swallowtail"
	brand.clip_text = true
	brand.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
	brand.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	brand.add_theme_font_size_override("font_size", _s(14))
	if _bold_font != null:
	...
	# Keep a compact, explicit game-state tooltip beside the reset action.
	...
	_reset_dialog.title = "Reset Swallowtail stats"
...
func _build_banner() -> void:
	...
	_banner.add_theme_constant_override("separation", _s(8))
	_banner.visible = false
	add_child(_banner)
	_banner.add_child(_make_mascot(BUTLER_WARNING, 40))
	var bubble := _make_speech(_sb_banner)
	_banner.add_child(bubble)
	...
	var details := _action_button("Details", _show_latest_error)
...
func _action_button(text: String, action: Callable) -> Button:
	...
	button.text = text
	button.add_theme_font_size_override("font_size", _s(10))
	_style_chip_button(button, false)
	button.pressed.connect(action)
	return button


func _build_actions() -> void:
	_connection_status = _make_body_text("Checking editor connection...")
	add_child(_connection_status)
	var actions := HFlowContainer.new()
	actions.add_theme_constant_override("h_separation", _s(6))
	actions.add_theme_constant_override("v_separation", _s(4))
	add_child(actions)
	actions.add_child(_action_button("Setup", _show_setup))
	actions.add_child(_action_button("Export activity", _choose_export))
	actions.add_child(_reset_button)

	_details_dialog = AcceptDialog.new()
	_details_dialog.title = "Swallowtail details"
	_details_dialog.min_size = Vector2i(_s(300), _s(220))
	_details_dialog.get_ok_button().text = "Close"
	_details_text = TextEdit.new()
	_details_text.editable = false
	_details_text.custom_minimum_size = Vector2(_s(280), _s(190))
	_details_text.wrap_mode = TextEdit.LINE_WRAPPING_BOUNDARY
	_details_dialog.add_child(_details_text)
	var copy_details := _details_dialog.add_button("Copy details", false, "details")
	copy_details.tooltip_text = "Copy the displayed diagnostic text."
	_copy_plan_button = _details_dialog.add_button("Copy plan", false, "plan")
	_copy_plan_button.tooltip_text = "Copy a one-step JSON plan for swallowtail automate --file. Review before running."
	_details_dialog.custom_action.connect(_on_details_action)
	add_child(_details_dialog)

	_export_dialog = FileDialog.new()
	_export_dialog.file_mode = FileDialog.FILE_MODE_SAVE_FILE
	_export_dialog.access = FileDialog.ACCESS_FILESYSTEM
	_export_dialog.filters = PackedStringArray(["*.json ; JSON activity report"])
	_export_dialog.file_selected.connect(_save_activity)
...
func _show_setup() -> void:
	_read_ports()
	_selected_error = {}
	_copy_plan_button.disabled = true
	_details_dialog.title = "Swallowtail connection setup"
	var http := "Disabled or not bound"
	if _http_port > 0:
		http = "http://127.0.0.1:%d/mcp" % _http_port
	_details_text.text = "Project: %s\n\nEditor WebSocket: %s\nOptional MCP HTTP: %s\n\nThe CLI connects for each command; zero connected peers is normal between calls.\n\nRun swallowtail doctor --json from this project to check installation and connection setup.\n\nCLI: swallowtail engine version\nMCP stdio: swallowtail serve\n\nGame direct server enabled: %s" % [ProjectSettings.globalize_path("res://"), "ws://127.0.0.1:%d" % _ws_port if _ws_port > 0 else "Not bound; check editor Output", http, str(ProjectSettings.get_setting("swallowtail/runtime/direct_server", false))]
	_details_dialog.popup_centered(Vector2i(_s(520), _s(400)))


func _show_latest_error() -> void:
	if not is_instance_valid(_router):
		return
	for entry: Dictionary in _router.stats_snapshot().get("recent", []):
		if not bool(entry.get("ok", true)):
			_show_error(entry)
			return


func _show_error(entry: Dictionary) -> void:
	_selected_error = entry.duplicate(true)
	_details_dialog.title = "Swallowtail: %s" % String(entry.get("method", "Error"))
	_details_text.text = JSON.stringify(entry, "  ")
	_copy_plan_button.disabled = not bool(entry.get("replay_available", false))
	_details_dialog.popup_centered(Vector2i(_s(520), _s(400)))


func _on_error_row(index: int) -> void:
	_show_error(_err_rows[index].get("entry", {}))


func _on_details_action(action: StringName) -> void:
	if action == &"details":
		DisplayServer.clipboard_set(_details_text.text)
	elif action == &"plan" and bool(_selected_error.get("replay_available", false)):
		DisplayServer.clipboard_set(JSON.stringify({"steps": [{"method": _selected_error["method"], "params": _selected_error["request_params"]}]}, "  "))


func _choose_export() -> void:
	if not is_instance_valid(_router):
		return
	_export_snapshot = {"schema_version": 1, "project_path": ProjectSettings.globalize_path("res://"), "captured_at": Time.get_datetime_string_from_system(true), "engine": Engine.get_version_info(), "activity": _router.stats_snapshot().duplicate(true)}
	_export_dialog.current_file = "swallowtail-activity.json"
	_export_dialog.popup_centered_ratio(0.65)


func _save_activity(path: String) -> void:
	var file := FileAccess.open(path, FileAccess.WRITE)
	if file != null:
		file.store_string(JSON.stringify(_export_snapshot, "  "))
		file.flush()
	...
	_selected_error = {}
	_copy_plan_button.disabled = true
	_details_dialog.title = "Activity export failed"
	_details_text.text = "Could not write the activity report to %s. Choose a writable location and try again." % path
	_details_dialog.popup_centered(Vector2i(_s(480), _s(280)))
...
func _build_errors_section(parent: VBoxContainer) -> void:
	...
	for _i in RECENT_ERRORS:
		...
		row.add_child(_action_button("View", _on_error_row.bind(_i)))
...
func _build_feed(parent: VBoxContainer) -> void:
	...
	var time_head := _make_micro("TIME", _s(COL_TIME))
	time_head.set_meta("gm_compact_hide", true)
	head.add_child(time_head)
	var group_head := _make_micro("GROUP", _s(COL_CHIP))
	group_head.set_meta("gm_compact_hide", true)
	head.add_child(group_head)
	...
	_feed_empty = HBoxContainer.new()
	_feed_empty.add_theme_constant_override("separation", _s(8))
	_feed_empty.add_child(_make_mascot(BUTLER_TIP, 40))
	var bubble := _make_speech(_sb_speech)
	bubble.add_child(_make_body_text("Ready when you are. Run a swallowtail command to see activity here."))
	_feed_empty.add_child(bubble)
...
func _make_mascot(path: String, logical_size: int) -> TextureRect:
	var portrait := TextureRect.new()
	# Decode directly: editor plugins can start before the first import scan.
	var image := Image.new()
	...
	portrait.custom_minimum_size = Vector2(_s(logical_size), _s(logical_size))
	portrait.size_flags_vertical = Control.SIZE_SHRINK_BEGIN
	portrait.mouse_filter = Control.MOUSE_FILTER_IGNORE
	return portrait


func _make_speech(style: StyleBoxFlat) -> PanelContainer:
	var bubble := PanelContainer.new()
	bubble.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	bubble.add_theme_stylebox_override("panel", style)
	bubble.draw.connect(_draw_speech_tail.bind(bubble, style))
	return bubble


func _draw_speech_tail(bubble: Control, style: StyleBoxFlat) -> void:
	var points := PackedVector2Array([
		Vector2(0, _s(12)), Vector2(-_s(6), _s(18)), Vector2(0, _s(24)),
	])
	bubble.draw_colored_polygon(points, style.bg_color)
	bubble.draw_polyline(points, style.border_color, _s(1), true)
...
## Compact chip button: hairline border, transparent fill, mulberry when
...
func _update_header(snap: Dictionary) -> void:
	_read_ports()
	_connection_status.text = "Editor ready on port %d." % _ws_port if _ws_port > 0 else "Editor connection unavailable. Check Setup and editor Output."
	...
	for e: Dictionary in recent:
		if bool(e.get("ok", true)):
			continue
		_banner_text.text = "Warning: %s failed %s" % [String(e.get("method", "")), _clock(int(e.get("ts", 0)))]
		_banner_text.tooltip_text = _banner_text.text
...
func _update_errors(snap: Dictionary) -> void:
	...
	for e: Dictionary in recent:
		...
		var failure: Dictionary = e.get("error", {})
		params_lbl.text = String(failure.get("message", params))
		params_lbl.tooltip_text = JSON.stringify(failure, "  ") if not failure.is_empty() else params
		params_lbl.visible = not params_lbl.text.is_empty()
		shown += 1
...
func _make_feed_row(entry: Dictionary) -> Control:
	...
	row.tooltip_text = "%s | %s | %s\n%s" % [method, _clock(int(entry.get("ts", 0))), _dur(ms), params]
	if not ok:
		row.tooltip_text += "\n" + String(entry.get("error", {}).get("message", "Failed call")) + "\nClick or press Enter for details."
		row.focus_mode = Control.FOCUS_ALL
		row.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
		row.gui_input.connect(_on_feed_error_input.bind(entry))
	...
	time_lbl.set_meta("gm_compact_hide", true)
	time_lbl.add_theme_font_size_override("font_size", _s(10))
	time_lbl.custom_minimum_size = Vector2(_s(COL_TIME), 0)
	time_lbl.clip_text = true
	_mono(time_lbl)
	...
	var chip := Label.new()
	chip.set_meta("gm_compact_hide", true)
	...
	method_lbl.set_meta("gm_method", method)
	method_lbl.set_meta("gm_failed", not ok)
	method_lbl.add_theme_font_size_override("font_size", _s(11))
	method_lbl.clip_text = true
	method_lbl.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
	...
	if ok or params.is_empty():
		...
		detail.text = _clip(String(entry.get("error", {}).get("message", params)), ERR_PARAM_CLIP)
	...
	_apply_feed_density(row, size.x < _s(340))
	return row


func _update_feed_density() -> void:
	if not _built:
		return
	var compact := size.x < _s(340)
	_apply_feed_density(_feed_head, compact)
	_apply_feed_density(_feed_rows, compact)


func _apply_feed_density(node: Node, compact: bool) -> void:
	if node is Control and node.has_meta("gm_compact_hide"):
		(node as Control).visible = not compact
	if node is Label and node.has_meta("gm_method"):
		(node as Label).text = ("Failed: " if compact and bool(node.get_meta("gm_failed")) else "") + String(node.get_meta("gm_method"))
	for child in node.get_children():
		_apply_feed_density(child, compact)


func _on_feed_error_input(event: InputEvent, entry: Dictionary) -> void:
...
func _on_cadence_draw() -> void:
	...
	for i in CADENCE_BUCKETS:
		...
		if not _cadence_empty:
			col = _col_accent if i >= CADENCE_HOT else _col_mid
...
func _on_group_bar_draw(bar: Control, idx: int) -> void:
	...
	if frac <= 0.0:
		return
	bar.draw_rect(Rect2(0, 0, w * frac, h), _col_accent if idx == 0 else _col_mid)
...
## panel with no second code path. Mulberry uses the docs' light/dark variants.
func _resolve_theme() -> void:
	...
	_col_accent = Color("81485f") if light else Color("dda6bb")
	_col_panel = base.lerp(Color("faf6ef") if light else Color("241e27"), 0.18)
	...
	_sb_banner.bg_color = _col_panel.lerp(_col_err, 0.10)
	_sb_banner.set_border_width_all(_s(1))
	_sb_banner.set_corner_radius_all(_s(10))
	_sb_banner.border_color = _col_err
	_sb_speech = _ensure_box(_sb_speech, _s(8), _s(7))
	_sb_speech.bg_color = _col_panel.lerp(_col_accent, 0.08)
	_sb_speech.set_border_width_all(_s(1))
	_sb_speech.set_corner_radius_all(_s(10))
	_sb_speech.border_color = _col_panel.lerp(_col_accent, 0.4)
	...
	_sb_btn_on.bg_color = _col_accent
	_sb_btn_on.set_border_width_all(_s(1))
	_sb_btn_on.border_color = _col_accent
...
## Shared box with scaled margins; speech bubbles add their own larger radius.
func _ensure_box(box: StyleBoxFlat, margin_h: int, margin_v: int) -> StyleBoxFlat:
	var sb := box
	if sb == null:
		sb = StyleBoxFlat.new()
	sb.set_corner_radius_all(_s(3))
...
func _read_ports() -> void:
	...
	f.close()
	if data is Dictionary and int(data.get("pid", 0)) == OS.get_process_id():
