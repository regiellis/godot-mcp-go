...
// runServe runs swallowtail as a Model Context Protocol server over stdio. It
...
func runServe(args []string) int {
	...
	fs.Usage = subHelp(fs, "run the stdio MCP server for AI clients",
		[]string{"swallowtail serve [--project DIR] [--port N] [--timeout 60s] [--typed=false]"},
		"Speaks MCP on stdin/stdout (logs on stderr). Point a client at it with swallowtail configure <client>.")
	...
	logf("swallowtail MCP server ready (stdio)")

	s.serve(os.Stdin)
	return 0
}

func logf(format string, a ...any) { fmt.Fprintf(os.Stderr, "[swallowtail serve] "+format+"\n", a...) }
...
type mcpServer struct {
	...
	operationContext context.Context // only accessed by the serialized request worker
...
func (s *mcpServer) reply(id json.RawMessage, result any, e *rpcErr) {
	...
	if call := s.calls[requestKey(id)]; call != nil && call.ctx.Err() != nil {
		return
	}
...
func (s *mcpServer) writeFrame(b []byte) {
	s.outMu.Lock()
...
func (s *mcpServer) initializeResult(params json.RawMessage) map[string]any {
	...
	if json.Unmarshal(params, &p) == nil && slices.Contains([]string{mcpProtocolVersion, "2025-03-26"}, p.ProtocolVersion) {
	...
	return map[string]any{
		...
		"serverInfo":   map[string]any{"name": "swallowtail", "version": cliVersion},
...
// steer, with the swallowtail skill carrying the detail.
const serverInstructions = "Drives a running Godot editor via the swallowtail addon. " +
	"Developed against Godot 4.7, with 4.3 to 4.8 supported in beta, where an older engine may skip a command group. " +
...
var godotRunTool = map[string]any{
	"name": "godot_run",
	"description": "Run any command against a running Godot editor (via the swallowtail addon) and return the JSON result. " +
		"Godot 4.7 is the development target, with 4.3 to 4.8 supported in beta. " +
...
// toolsList refreshes from live docs before returning generic and typed tools.
...
func (s *mcpServer) fetchTypedTools() (ok bool) {
	defer func() {
		if !ok {
			s.typedFetched = false
			s.typedTools = nil
			s.nameToMethod = nil
		}
	}()
	resolved, err := client.ResolvePort(s.flagPort, s.cwd)
	if err != nil {
		logf("typed tools: %v", err)
		return false
	}
	ctx, cancel := context.WithTimeout(s.requestContext(), 5*time.Second)
	defer cancel()
...
// A successful generic editor/resource call also refreshes catalog discovery.
func (s *mcpServer) maybeUpgradeTypedTools() {
	s.refreshTypedTools()
}

// Refresh on list, before typed dispatch, and after generic editor calls.
// Resolve discovery again each time: both the editor port and its catalog can
// change while this stdio connection stays alive. Never retain stale tools on
// failure, and never notify for an identical catalog.
func (s *mcpServer) refreshTypedTools() {
	if !s.typed {
		return
	}
	before, _ := json.Marshal(s.typedTools)
	s.fetchTypedTools()
	after, _ := json.Marshal(s.typedTools)
...
func (s *mcpServer) resourcesRead(msg rpcMsg) {
	...
	ctx, cancel := context.WithTimeout(s.requestContext(), s.timeout)
	defer cancel()
...
func (s *mcpServer) toolsCall(msg rpcMsg) {
	...
	if isGame {
		resolved, resolveErr = client.ResolveGamePort(0, s.cwd)
	} else {
		portRes = client.ResolvePortSource(s.flagPort, s.cwd)
		resolved = portRes.Port
		resolveErr = portRes.Err
	}
	if resolveErr != nil {
		s.reply(msg.ID, toolResult(resolveErr.Error(), true), nil)
		return
	}

	ctx, cancel := context.WithTimeout(s.requestContext(), methodTimeout(method, params, s.timeout))
	defer cancel()
	...
	result, err := client.CallWithProgress(ctx, resolved, method, params, func(raw json.RawMessage) { s.forwardProgress(msg.ID, raw) })
	if err != nil {
...
func gameDialErrorPayload(port int) map[string]any {
	return map[string]any{
		...
		"checks": []string{
			...
			"the swallowtail/runtime/direct_server project setting is enabled",
		},
	}
}
