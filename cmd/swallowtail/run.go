...
func runRun(args []string) int {
	...
	fs.Usage = subHelp(fs, "run the game standalone, with no editor",
		[]string{"swallowtail run [scene] [--headless] [--debug-collisions] [--wait=false] [--project DIR] [--json]"},
		`Starts godot --path <project> [scene] with the flags below, detached, and sends
its stdout and stderr to <project>/.godot/swallowtail-run.log. The scene is
...
server binds, then reports the port to drive with swallowtail --game. That channel
needs the swallowtail/runtime/direct_server project setting; without it the command
...
for one-word flags and their values.`)
	...
	if d, derr := client.ReadGameDiscovery(root); derr == nil && d.Port > 0 && client.PIDAlive(d.PID) {
		fmt.Fprintf(os.Stderr, "%s a game from this project is already running (pid %d, port %d); not starting another. Close it, or drive it with swallowtail --game.\n",
	...
	direct := projectBoolSetting(root, "swallowtail", "runtime/direct_server")
	...
	switch {
	case !direct:
		res.Hint = "swallowtail project set-setting --path swallowtail/runtime/direct_server --value true"
	...
	res.GamePID = d.PID
	res.Connected = true
	res.Hint = fmt.Sprintf("swallowtail --game runtime tree   (port %d)", d.Port)
