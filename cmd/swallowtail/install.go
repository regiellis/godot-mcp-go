...
func runInstall(args []string) int {
	...
	from := fs.String("from", "", "addon source dir (default: addons/swallowtail beside the binary, or project/addons/swallowtail in a checkout)")
	skillFrom := fs.String("skill-from", "", "skill source dir (default: skills/swallowtail beside the binary or repo root)")
	skill := fs.Bool("skill", true, "install the agent skill into <project>/.claude/skills/swallowtail (use --skill=false to skip)")
	...
	fs.Usage = subHelp(fs, "copy the addon + agent skill into a Godot project",
		[]string{"swallowtail install [--project DIR] [--from DIR] [--skill-from DIR] [--skill] [--enable] [--force]"})
	...
	addonSrc, tried := resolveAsset(*from, "plugin.cfg",
		[]string{"addons", "swallowtail"},            // release archive: beside the binary
		[]string{"project", "addons", "swallowtail"}, // repo checkout: under project/
	)
	if addonSrc == "" {
		...
		fmt.Fprintln(os.Stderr, "or pass --from with the path to an addons/swallowtail directory.")
		return 1
	}

	if pathExists(filepath.Join(root, "addons", "godot_mcp")) {
		fmt.Fprintln(os.Stderr, "Legacy addon found. Run swallowtail migrate --project", root, "before installing Swallowtail; --force does not bypass migration.")
		return 1
	}
	addonDst := filepath.Join(root, "addons", "swallowtail")
	...
	if *skill {
		...
		skillSrc, skillTried := resolveAsset(*skillFrom, "SKILL.md", []string{"skills", "swallowtail"})
		if skillSrc == "" {
			fmt.Fprintln(os.Stderr, "install: skill source not found, skipping it. Looked in:")
			for _, p := range skillTried {
				fmt.Fprintf(os.Stderr, "  %s\n", p)
			}
			fmt.Fprintln(os.Stderr, "Pass --skill-from with the path to a skills/swallowtail directory, or --skill=false to silence this.")
		} else {
			skillDst := filepath.Join(root, ".claude", "skills", "swallowtail")
	...
	if *enable {
		...
		fmt.Println("Next: open the project in Godot 4.7 and enable Swallowtail in Project Settings > Plugins.")
	}
	return 0
}
...
const pluginEntry = "res://addons/swallowtail/plugin.cfg"
