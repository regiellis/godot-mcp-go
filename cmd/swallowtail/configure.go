...
func runConfigure(args []string) int {
	...
	name := fs.String("name", "swallowtail", "MCP server key to write")
	force := fs.Bool("force", false, "replace an existing server entry of the same name")
	fs.Usage = subHelp(fs, "point an AI client at Swallowtail's optional stdio MCP server",
		[]string{"swallowtail configure <client> [--project DIR] [--config-dir DIR] [--global] [--print] [--name NAME] [--force]"},
	...
	if perr != nil {
		...
		if nested := nestedProjects(absStart); len(nested) > 0 {
			...
			for _, n := range nested {
				fmt.Fprintf(os.Stderr, "  swallowtail configure %s --project %s --config-dir %s\n", clientName, n, absStart)
			}
		}
		return 1
	}
...
func configureJSON(path, parentKey, name, command string, cmdArgs []string, vscode, force, printOnly bool, label string) int {
	...
	if name == "swallowtail" && parent["godot-mcp"] != nil {
		fmt.Fprintln(os.Stderr, "configure: legacy godot-mcp entry exists. Use --name godot-mcp --force to update its executable without adding a duplicate server, or remove that entry before configuring the new name.")
		return 1
	}
...
func configureTOML(path, name, command string, cmdArgs []string, force, printOnly bool, label string) int {
	...
	if name == "swallowtail" && tomlHasSection(existing, "godot-mcp") {
		fmt.Fprintln(os.Stderr, "configure: legacy godot-mcp entry exists. Use --name godot-mcp --force to update it without adding a duplicate server.")
		return 1
	}
