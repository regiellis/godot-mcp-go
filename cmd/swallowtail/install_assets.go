...
func runInstallAssets(args []string) int {
	...
	from := fs.String("from", "", "asset source dir (default: addons/swallowtail/assets next to the binary)")
	...
	fs.Usage = subHelp(fs, "copy bundled CC0 asset packs into a project",
		[]string{
			"swallowtail install-assets [--project DIR] [--dest assets/vendor] [--pack NAME] [--force]",
	...
	src, tried := resolveAsset(*from, ".gdignore",
		[]string{"addons", "swallowtail", "assets"},            // release archive
		[]string{"project", "addons", "swallowtail", "assets"}, // repo checkout
	)
	if src == "" {
		...
		fmt.Fprintln(os.Stderr, "Run it from the extracted release folder, or pass --from with the path to addons/swallowtail/assets.")
		return 1
	}
