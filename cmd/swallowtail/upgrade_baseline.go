...
// build with swallowtail/runtime/direct_server on, which is what swallowtail run
// already requires.
...
func runUpgradeBaseline(args []string) int {
	...
	fs.Usage = subHelp(fs, "record how the game behaves under the old binary",
		[]string{"swallowtail upgrade baseline --old-godot PATH [--scenario FILE] [--frames 300] [--json]"},
...
which needs the swallowtail/runtime/direct_server project setting and a debug
...
func runUpgradeDrive(root, relDir, phase, bin, scenarioPath string, frames int, timeout time.Duration, asJSON, quiet bool) int {
	...
	// won. Same no-stacking rule swallowtail run applies.
	...
	capt.Mode = "scenario"
	if !projectBoolSetting(root, "swallowtail", "runtime/direct_server") {
		fmt.Fprintf(os.Stderr, "%s replaying a scenario needs the game's direct channel, which is off for this project.\n", ui.Err.Fail("error:"))
		fmt.Fprintln(os.Stderr, "Turn it on and commit the change: swallowtail project set-setting --key swallowtail/runtime/direct_server --value true")
		return 1
	}
...
func replayStep(game *gameSession, i int, st scenarioStep, dir, userDir string, capt *driveCapture) driveStep {
	...
	case "screenshot":
		...
		save := "user://swallowtail-upgrade/" + name
		if _, err := game.call("runtime.screenshot", map[string]any{"save_path": save}); err != nil {
			out.Detail = err.Error()
			return out
		}
		dst := filepath.Join(dir, name)
		src := filepath.Join(userDir, "swallowtail-upgrade", name)
