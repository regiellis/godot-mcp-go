...
//
// The rollback unit is the checkpoint plus captured files. Each addon
...
// scene's node identities and transforms; the file snapshots restore only the
// plan's paths. The git tag remains a recovery point for manual intervention.
...
func runUpgradeFix(args []string) int {
	...
	fs.Usage = subHelp(fs, "apply one category, prove it, and keep or restore",
		[]string{"swallowtail upgrade fix --category NAME [--dry-run] [--godot PATH] [--scenario FILE] [--json]"},
...
command list for the ones that do not.`)
	...
	if len(recovery.Scenes) > 0 {
		if err := requireSavedScenes(sess); err != nil {
			fmt.Fprintln(os.Stderr, "error:", err)
			return 1
		}
	}
	scene, captured, err := captureFixCheckpoint(sess, label, len(recovery.Scenes) > 0)
	if err != nil {
		fmt.Fprintln(os.Stderr, "error: cannot capture recovery checkpoint:", err)
		return 1
	}
	if !captured {
		report.Checkpoint = ""
	}
	recovery.Scene = scene
	...
	if berr != nil {
		...
		report.Reason = "capturing proof failed: " + berr.Error()
	...
	if report.Reason != "" {
		...
		restoreFix(root, sess, &report)
		emitFix(report, *asJSON, cat+" failed proof: "+report.Reason)
		return 1
	}
...
func emitFix(report fixReport, asJSON bool, line string) {
	...
	if report.Restored {
		rows = append(rows, [2]string{"restored", "checkpoint and planned files verified"})
	}
	if len(report.RecoveryErrors) > 0 {
		rows = append(rows, [2]string{"recovery incomplete", strings.Join(report.RecoveryErrors, "; ")})
		line += "; recovery incomplete (see recovery_errors)"
	}
	if report.Commit != "" {
