...
func TestCollectScriptsAddonsWhenNamed(t *testing.T) {
	...
	want := []string{"res://addons/swallowtail/plugin.gd"}
	if strings.Join(got, ",") != strings.Join(want, ",") {
...
func TestUnderAddons(t *testing.T) {
	...
	cases := map[string]bool{
		...
		filepath.Join(root, "addons", "swallowtail"):      true,
...
// so `swallowtail check res://scripts` works from anywhere.
