...
func runDoctor(args []string) int {
	...
	fs.Usage = subHelp(fs, "environment preflight",
		[]string{"swallowtail doctor [--project DIR] [--json]"},
...
// checkGodotBinary reports the launcher `swallowtail launch` would run (see
...
// checkAddonInstalled reports whether addons/swallowtail/plugin.cfg is present.
func checkAddonInstalled(root string) doctorCheck {
	p := addonConfigPath(root)
	if fileExists(p) {
		return doctorCheck{"addon installed", statusOK, p + " present"}
	}
	return doctorCheck{"addon installed", statusFail,
		"addon not found. Run: swallowtail install --project " + root}
}
...
func checkAddonEnabled(root string) doctorCheck {
	...
	if strings.Contains(string(data), pluginEntry) || strings.Contains(string(data), "res://addons/godot_mcp/plugin.cfg") {
		return doctorCheck{"addon enabled", statusOK, "automation addon enabled in project.godot"}
	}
	return doctorCheck{"addon enabled", statusWarn,
		"plugin not enabled. Run: swallowtail install --enable, or enable Swallowtail in Project Settings > Plugins"}
}
...
func checkGameAutoloads(root string) doctorCheck {
	...
	for _, entry := range gameAutoloads {
		...
		case strings.TrimPrefix(v, "*") != entry[1] && strings.TrimPrefix(v, "*") != strings.ReplaceAll(entry[1], "addons/swallowtail/", "addons/godot_mcp/"):
	...
	if len(missing) > 0 {
		return doctorCheck{"game autoloads", statusWarn,
			fmt.Sprintf("%s missing from project.godot; runtime/input commands need them. Run: swallowtail install --project %s --enable --force",
...
// SWALLOWTAIL_PORT (or its legacy alias) pins it; else the project network/port;
...
func checkPortConfig(root string) doctorCheck {
	env := strings.TrimSpace(client.Env("GODOT_MCP_PORT"))
	envName := "SWALLOWTAIL_PORT"
	if os.Getenv(envName) == "" {
		envName = "GODOT_MCP_PORT"
	}
	...
	case env != "" && hasPin:
		if env == strconv.Itoa(pin) {
			return doctorCheck{"port config", statusOK,
				fmt.Sprintf("%s=%s (matches project network/port pin)", envName, env)}
		}
		return doctorCheck{"port config", statusWarn,
			fmt.Sprintf("%s=%s overrides project network/port pin=%d (they disagree)", envName, env, pin)}
	case env != "":
		return doctorCheck{"port config", statusOK, envName + "=" + env}
	case hasPin:
		return doctorCheck{"port config", statusOK,
			fmt.Sprintf("project network/port pin=%d", pin)}
	default:
		return doctorCheck{"port config", statusOK, "auto (9080-9095)"}
	}
}

// checkEditor runs the same liveness diagnosis as `swallowtail status`. Only
...
func readPortPin(root string) (int, bool) {
	...
	wanted := "godot_mcp"
	if strings.Contains(string(data), "[swallowtail]") {
		wanted = "swallowtail"
	}
	section := ""
...
func emitDoctor(checks []doctorCheck, asJSON bool) int {
	...
	if asJSON {
		...
		fmt.Println(ui.Out.Heading("swallowtail doctor") + ": environment preflight")
		fmt.Println()
...
func addonConfigPath(root string) string {
	path := filepath.Join(root, "addons", "swallowtail", "plugin.cfg")
	if !fileExists(path) {
		return filepath.Join(root, "addons", "godot_mcp", "plugin.cfg")
	}
	return path
}
