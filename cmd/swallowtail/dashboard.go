...
func runDashboard(args []string) int {
	...
	fs.Usage = subHelp(fs, "live stats web dashboard",
		[]string{"swallowtail dashboard [--port 8090] [--project DIR] [--addon-port N]"})
	...
	resolve := func() (int, error) { return client.ResolvePort(*addonPort, cwd) }
	if err := client.ValidatePort(*httpPort, "--port"); err != nil {
