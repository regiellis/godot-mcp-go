...
func godotLogPath(root, name string) string {
	return filepath.Join(root, ".godot", "swallowtail-"+name+".log")
}
...
func projectBoolSetting(root, section, key string) bool {
	...
	if !ok && section == "swallowtail" {
		v, ok = projectSetting(root, "godot_mcp", key)
	}
...
func emitResult(name string, res any, asJSON bool, line string, rows [][2]string) {
	...
	fmt.Println(ui.Out.Heading("swallowtail "+name) + ": " + line)
	fmt.Println()
	w := 0
	for _, r := range rows {
		w = max(w, len(r[0]))
