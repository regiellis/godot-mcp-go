...
func TestBuildToolRetainsNestedSchema(t *testing.T) {
	doc := commandDoc{Params: []paramDoc{{Name: "steps", Type: "Array", Required: true,
		Schema: map[string]any{"minItems": 1, "items": map[string]any{"oneOf": []any{map[string]any{"type": "object", "additionalProperties": false}}}}}}}
	tool := buildTool("test_run_scenario", "test.run_scenario", doc)
	encoded, err := json.Marshal(tool)
	if err != nil || !strings.Contains(string(encoded), `"additionalProperties":false`) || !strings.Contains(string(encoded), `"minItems":1`) {
		t.Fatalf("nested schema lost: %s (%v)", encoded, err)
	}
}

func TestInitializeSupportedVersions(t *testing.T) {
	for _, version := range []string{"2025-06-18", "2025-03-26", "unsupported-probe", ""} {
		t.Run(version, func(t *testing.T) {
			params, _ := json.Marshal(map[string]string{"protocolVersion": version})
			got := (&mcpServer{}).initializeResult(params)["protocolVersion"]
			want := version
			if version == "" || version == "unsupported-probe" {
				want = mcpProtocolVersion
			}
			if got != want {
				t.Fatalf("negotiated %v, want %s", got, want)
			}
		})
	}
}
