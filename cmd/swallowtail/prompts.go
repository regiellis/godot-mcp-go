...
// from the swallowtail agent skill. Unlike typed tools (built from the addon's live
// param docs), prompts carry embedded text and are served regardless of editor
// availability, because the binary is distributed standalone and does no runtime file reads.
...
var prompts = []mcpPrompt{
	{
		...
		description: "The core swallowtail playbook: ground on the live 4.7 engine API before acting, and follow the durable editor rules.",
...
// The four prompt bodies below are distilled from skills/swallowtail/SKILL.md and the
// root context doc: tight, self-contained instruction blocks, not verbatim dumps.

const discoverThenDrivePrompt = "Drive a running Godot editor (4.7+) through the swallowtail addon with the discover-then-drive loop. " +
...
const spatialPlacementPrompt = "Place 3D objects with the anchor / read-back / verify discipline. You cannot perceive 3D reliably from one screenshot, and absolute-coordinate math drifts. " +
	...
	"4. Face with spatial.look_at (engine math), never hand-computed Euler, because hand-rolled angles run about 20 degrees off.\n" +
	"5. Verify numerically, not by one screenshot. spatial.relate / spatial.lint after a placement (center_delta.x = 0, gap.y = 0 means touching, not sunk). If you genuinely need eyes, teleport the camera to several vantages, since one frame hides a centimeter-scale error.\n\n" +
...
const launchRecoveryPrompt = "Recover an unreachable Godot editor from its discovery verdict, not by relaunching blindly. When a command fails because the editor isn't reachable, " +
	"run the swallowtail status (or doctor) preflight first. It reads the addon's discovery file and returns a verdict:\n\n" +
	...
	"Launch with `swallowtail launch [--headless]`, which applies these verdicts itself and logs the editor's output to <project>/.godot/swallowtail-launch.log (fall back to `godot --path <project> --editor` when the CLI is not installed). Relaunch AT MOST ONCE; if it is still unreachable after one attempt, stop and report. Do not loop launches. " +
