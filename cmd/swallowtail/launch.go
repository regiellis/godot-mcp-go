...
func runLaunch(args []string) int {
	...
	fs.Usage = subHelp(fs, "launch one editor for this project",
		[]string{"swallowtail launch [--project DIR] [--headless] [--wait=false] [--timeout 90s] [--godot PATH] [--json]"},
...
<project>/.godot/swallowtail-launch.log, never to this terminal.
...
rather than the editor itself. Run status afterwards for the editor's own pid.`)
	...
	if st.Verdict == client.VerdictConfigError {
