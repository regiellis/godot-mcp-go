...
func runExport(args []string) int {
	...
	fs.Usage = subHelp(fs, "export a preset headlessly and report what it produced",
		[]string{"swallowtail export <preset> [--debug|--pack|--patch] [--out PATH] [--project DIR] [--timeout 20m] [--json]"},
...
export template looks like: check swallowtail export info and swallowtail doctor.

Godot's stdout and stderr go to <project>/.godot/swallowtail-export.log, with the
ERROR and WARNING lines parsed into the result.

The addon's export group keeps this name: swallowtail export list-presets, export
project and export info still reach the editor. A preset called one of those
three is reachable as swallowtail export --preset info.`)
	...
	case presetName == "" && len(positional) != 1:
		fmt.Fprintf(os.Stderr, "%s export takes one preset name (swallowtail export \"Windows Desktop\")\n", ui.Err.Fail("error:"))
		return 2
	case presetName == "":
...
func exportOutcome(res exportResult, timedOut bool, timeout time.Duration) (string, int) {
	...
	case !res.Exists:
		return fmt.Sprintf("godot exited 0 but wrote nothing to %s. Check export templates (swallowtail export info, swallowtail doctor) and the preset's own settings; see %s",
...
func readExportPresets(root string) ([]exportPreset, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("no export_presets.cfg at %s. Configure an export in Project > Export first (swallowtail export list-presets lists them from a running editor)", path)
		}
		return nil, err
	}
