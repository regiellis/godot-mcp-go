...
// skills/swallowtail/porting-godot-versions.md, section "The upgrade pipeline":
...
func upgradeUsage() {
	p := ui.Err
	w := os.Stderr
	fmt.Fprintln(w, p.Heading("swallowtail upgrade")+": port a 4.3+ project to a newer Godot, one gated phase at a time")
	fmt.Fprintln(w)
	fmt.Fprintln(w, p.Heading("Usage:"))
	fmt.Fprintln(w, "  "+tintSlots("swallowtail upgrade <phase> [flags]", p))
	phases := [][2]string{
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Per-phase flags: swallowtail upgrade <phase> --help")
}
...
func printTodoTable(counts map[string]int, files map[string]map[string]int) {
	...
	for _, c := range cats {
		applies := "report only"
		if isFixable(c) {
			applies = "swallowtail upgrade fix --category " + c
		}
