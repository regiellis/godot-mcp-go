...
func runUpgradePreflight(args []string) int {
	...
	fs.Usage = subHelp(fs, "audit the tree cold, before an editor has ever opened it",
		[]string{"swallowtail upgrade preflight [--project DIR] [--old-godot PATH] [--godot PATH] [--json]"},
...
is the same check swallowtail check runs.
...
in project.godot before the first launch. swallowtail upgrade verify reverts that
commit.
...
Writes `+"`"+upgradeReportDir+"/preflight.json`"+`.`)
	...
	if !*asJSON && ui.IsTerminal(os.Stdout) {
		...
		fmt.Println()
		fmt.Println(ui.Out.Dim("Next: swallowtail upgrade baseline --old-godot <path> [--scenario FILE]"))
	}
	return 0
}

// coldParseSweep runs the same per-file --check-only parse swallowtail check runs
