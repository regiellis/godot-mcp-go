...
func runImport(args []string) int {
	...
	fs.Usage = subHelp(fs, "import the project's assets headlessly",
		[]string{"swallowtail import [--project DIR] [--timeout 10m] [--godot PATH] [--json]"},
...
swallowtail import reimport --path res://....

Godot's stdout and stderr go to <project>/.godot/swallowtail-import.log, with the
ERROR and WARNING lines parsed into the result.

The addon's import group keeps this name: swallowtail import info, import set and
import reimport still reach the editor.`)
	...
	if st := client.Diagnose(root, 0); st.Verdict == client.VerdictRunning {
		...
		fmt.Fprintln(os.Stderr, "Close that editor and retry, or reimport in place: swallowtail import reimport --path res://your/file.png")
		return 1
	}
