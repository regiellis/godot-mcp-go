# Installing swallowtail

Godot 4.7 is the development target; Godot 4.3 through 4.8 have beta support. Individual commands may require newer APIs. Use a prebuilt release bundle or build with Go 1.26+. No agent or AI service is required. Add `--skill=false` to `install` to skip the optional agent skill.

Follow the [terminal quickstart](https://regiellis.github.io/godot-mcp-go/docs/quickstart) for a complete first run.
...
**Run it from the unpacked bundle**, where the binary sits beside `addons/` and `skills/`. That is how it finds what to copy: move the binary somewhere on its own first and `install` has no addon to install, and says so. If you have already moved it, point at the pieces with `--from DIR` (an `addons/swallowtail` directory) and `--skill-from DIR` (a `skills/swallowtail` directory).
...
Copy the `addons/swallowtail/` folder into your Godot project so you end up with:

```
<your project>/addons/swallowtail/
```

Then in Godot: **Project → Project Settings → Plugins → Swallowtail → Enable**.
...
(Standalone `swallowtail-addon_<version>.zip` extracts the `swallowtail` folder directly, so drop it into your project's `addons/`.)
...
```
<your project>/.claude/skills/swallowtail/SKILL.md     # project-scoped
# or, for all projects:
~/.claude/skills/swallowtail/SKILL.md                  # global
```
...
```sh
swallowtail install-assets --project /path/to/your/project
```
...
- **`kenney_prototype_textures`** is Kenney's [Prototype Textures](https://kenney.nl/assets/prototype-textures) (CC0): grid/checker greybox skins in per-colour folders (`PNG/Dark`, `Green`, `Orange`, `Purple`, `Red`, `Light`). Apply one to a blockout mesh as a triplanar `StandardMaterial3D`. The level-design skill (`skills/swallowtail/level-design.md`) has the recipe and a greybox colour language.

## Pinning the MCP port (optional)

By default the addon binds the first free port in **9080-9095** and writes it to `<project>/.godot/swallowtail.json`, so the CLI auto-discovers it and you rarely need to think about ports. Running **two projects at once**, pin each to a distinct port so they never contend:

**Project → Project Settings → General → Swallowtail → Network → Port** (turn on *Advanced Settings* if you don't see it). Set it to e.g. `9091`; `0` means auto-pick. The value is saved in that project's `project.godot`, and the server rebinds to it the next time the plugin loads (toggle it off/on under *Plugins*, or run `swallowtail editor reload-plugin`). `SWALLOWTAIL_PORT` in the environment still overrides everything. The default (`0`) is never written to `project.godot`, so leaving it alone keeps the file clean.

The **streamable-HTTP MCP endpoint** works the same way in its own range (**9100-9115**, recorded as `http_port` in the discovery file): pin it via **Network → Http Port** or `SWALLOWTAIL_HTTP_PORT`, turn it off via **Network → Mcp Http**, and set **Network → Http Typed** to off to expose only the generic `godot_run` tool to tool-limited clients.

## Adding your own commands (optional)

Extend the MCP with project-specific commands without forking the addon: drop a `.gd` file into a **`res://mcp_commands/`** folder in your project. On plugin enable the addon scans that folder and registers your commands next to the built-ins, so they show up in `swallowtail help <group>`, `swallowtail engine commands`, and the CLI automatically. No Go or addon changes.
...
Then `swallowtail custom ping` and `swallowtail custom echo --message hi` work. That is the shape; the [Add your own commands](https://regiellis.github.io/godot-mcp-go/docs/extending) page carries a worked example that earns its place (a sweep for `res://` references that no longer resolve) plus the two precautions a command that writes files has to take. A file that fails to load or lacks `get_commands()` is skipped with a warning (it never breaks the plugin), and a name that collides with a built-in is skipped, because built-ins win. Editing a command file needs a full editor restart to recompile (reloading the plugin re-runs registration but doesn't re-parse changed scripts).

## Driving a standalone game (optional)

`runtime`/`input` commands normally reach the running game through the open editor. To drive a game with **no editor at all** (QA rigs, packaged dev builds): enable **Project → Project Settings → Swallowtail → Runtime → Direct Server**, run a **debug build** of the game, then:
...
The game hosts its own `127.0.0.1`-only server (ports 9200-9215; `SWALLOWTAIL_GAME_PORT` pins one) and writes a discovery file in its `user://` dir, so `--game` finds it automatically from inside the project. Release exports never host this server. It is hard-gated to debug builds.

## Verify

1. Open your project in Godot 4.7+ (`godot`, or `swallowtail launch` from inside the project, which starts one editor and never a second) and enable the plugin.
...
If the CLI can't connect, run `swallowtail doctor` (from inside the project, or with `--project DIR`), which checks the godot binary, the addon install/enable state, the effective port, and whether an editor is actually reachable. Or pass `--port 9080` explicitly.

Four subcommands need no editor at all, because they run the engine themselves. On a fresh clone `swallowtail import` builds the `.godot/` import cache (it refuses while an editor is open on the project, since both own that cache), `swallowtail check <file|dir>...` parses `.gd` files and exits non-zero on the first that fails, `swallowtail export "<preset>"` runs a headless export and reports what landed, and `swallowtail run [scene]` starts the game standalone. Each logs the engine's output under `<project>/.godot/` and reports the errors it parsed out of it.

Moving an existing 4.3 or newer project to a newer Godot has its own subcommand: `swallowtail upgrade preflight|baseline|open|fix|verify`. Each phase writes a report under `<project>/.godot/upgrade/` and stops, both binaries are named explicitly (`--old-godot`, `--godot`), and the port runs on its own branch with a tag to fall back to. Read the [Porting between Godot versions](https://regiellis.github.io/godot-mcp-go/docs/guides/porting-godot-versions) guide first.
...
To verify a build, string-scan the exported `.pck`. It should contain no `swallowtail` or `MCPGame` matches. The [Shipping and export guide](https://regiellis.github.io/godot-mcp-go/docs/guides/shipping-export) covers the receipt scan and the rest of the release pipeline.
