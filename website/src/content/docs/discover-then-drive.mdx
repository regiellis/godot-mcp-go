...
swallowtail engine search --query navigation_layer

# the full API of one class, optionally filtered
swallowtail engine class-info --class CharacterBody3D --filter velocity

# registered singletons and global class_name scripts
swallowtail engine singletons
swallowtail engine script-classes
```
...
swallowtail engine doc-search --query "wrap text"

# then read the meaning, class-level or one member
swallowtail engine docs --class Label --member autowrap_mode
swallowtail engine docs --class Node --member queue_free
```
...
swallowtail node set --node-path Player --property motion_mode --value 1

# arbitrary GDScript, editor-side or in the running game
swallowtail editor run-script --code "print(ProjectSettings.get_setting('application/config/name'))"
```
