...
Structure answers what exists; the docs answer what it does. `engine docs` serves the running build's own documentation prose — the same text as the editor's Help panel, read from the editor's doc cache — and `engine doc-search` searches it by concept, for when you know the behavior you want but not the term:
