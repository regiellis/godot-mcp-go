...
swallowtail exposes every command as a typed MCP tool by default, and tool schemas are context: an MCP client carries its tool list into every request. That makes the size of the list a real cost, and a project that ships 330 commands owes its users a measured number. This page is that number, how it was measured, and what it means for the model you drive this with.

<Callout type="important" title="The short version">
  The full typed surface measures **about 52,000 tokens**. That is sized for big-context frontier models with 200K to 1M token windows and prompt caching, and those carry it comfortably. A small local model will not, and this project does not aim to serve one. If your client is context-tight, `serve --typed=false` collapses the surface to a single ~470-token tool, and the CLI needs no schemas at all.
</Callout>
...
**Prompt caching absorbs the recurring cost.** The tool list is part of the stable request prefix, so after the first request it is served from cache. On Claude's API, cached input bills at roughly a tenth of the base input rate (checked 2026-08-05; other providers price caching similarly). Catalog changes and editor availability can change the list mid-session. `serve` refreshes live definitions and emits a notification only when they differ, including when tools disappear during an outage or return after a restart. Sessions with long idle gaps can re-pay the cache write, which bills a fraction above the uncached rate for that one request.
...
| A client that caps tool count, or a smaller model | `serve --typed=false` (stdio) or the `swallowtail/network/http_typed` setting (HTTP endpoint): one generic tool, ~470 tokens |
| A terminal-capable agent, no MCP at all | The CLI, self-describing via `swallowtail help all` and per-command `--help`, zero schemas |
...
The single-tool mode still reaches all 330 commands: `godot_run` proxies any of them, and the model grounds itself with `engine.search` / `engine.class_info` instead of reading schemas. That is the same [discover-then-drive loop](/docs/discover-then-drive) the typed mode uses. What it gives up is the schema hints.
