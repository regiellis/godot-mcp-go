...
description: "swallowtail upgrade runs a 4.3-and-up port as five gated phases: audit the tree cold, record a baseline drive, harvest seven sources after the first open, apply the mechanical fixes with a proof behind each, and replay the drive to compare."
---
...
`swallowtail upgrade <phase>` runs that as five phases. Each does its work, writes a report under `<project>/.godot/upgrade/`, and stops. Nothing runs the next phase for you, because every phase ends with a report for you to review.
...
```bash
swallowtail install --project . --enable
git commit -am "chore: install swallowtail addon"

swallowtail upgrade preflight --old-godot /path/to/godot-4.4 --godot /path/to/godot-4.7
swallowtail upgrade baseline  --old-godot /path/to/godot-4.4 --scenario drive.json
swallowtail upgrade open      --godot /path/to/godot-4.7

swallowtail upgrade fix --category tilemap --dry-run
swallowtail upgrade fix --category tilemap --godot /path/to/godot-4.7

swallowtail upgrade verify --godot /path/to/godot-4.7
```
...
The resave lands as its own commit, so a later fix has a well-defined state to restore to. The editor stays up afterwards, because `fix` and `verify` drive it; close it yourself once the port is done (`swallowtail status` names its pid).
...
```bash
swallowtail upgrade fix --category export_file --dry-run
swallowtail upgrade fix --category export_file --godot /path/to/godot-4.7
```
...
Recovery restores only the files captured for that operation and preserves
unrelated files. It refreshes the editor's scene tabs, loaded scripts, and project
settings as well as disk contents. A restore that fails is reported explicitly;
inspect that error before retrying the migration.
...
Everything else a harvest found is a report for you to act on, and `fix` says so rather than guessing. The rename table currently ships report-only: every symbol the 4.3-to-4.7 range changed either drops an argument or has no replacement, and neither is a text substitution.
