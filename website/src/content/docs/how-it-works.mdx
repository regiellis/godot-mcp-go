...
- **The ports are unauthenticated by design.** Any process on the machine can drive the editor — that is what lets `godot-mcp project info` work with no keys, config, or pairing step. Commands reach `editor.run_script`, so "can connect" means "can run code in the editor".
...
That last point is a **known and accepted limitation**. Closing it means putting a shared secret in the connect URL, which every client — the CLI, `serve`, and anything you script yourself — would have to carry and keep in sync; on a private-network workstation that is a lot of moving parts protecting a small surface.

If your situation is different — a shared or untrusted network, a browser session you do not control — quit the editor when you are not driving it, since the servers exist only while it runs. `godot_mcp/network/mcp_http` turns the HTTP endpoint off independently, and disabling the plugin stops both.
