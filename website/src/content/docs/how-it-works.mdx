...
Godot's own command line (`godot --headless`, `--export-release`, `--script`) **starts a fresh engine process** each time, which is the right tool for CI exports and batch scripts and has no notion of a session. swallowtail is the other half: it **connects to the editor already running**, reaching the open scene, the current selection, the undo history, and the game that editor is playing. Different jobs; use both. swallowtail itself launches the editor through Godot's CLI when needed.
...
- The CLI resolves the port from `<project>/.godot/swallowtail.json` (written by the addon on bind), so the common case needs no flags.
...
swallowtail is a **local dev bridge on a workstation you trust**, and that assumption is load-bearing. While the editor is open with the plugin enabled:

- **The ports are unauthenticated by design**. Any process on the machine can drive the editor, which is what lets `swallowtail project info` work with no keys, config, or pairing step. Commands reach `editor.run_script`, so "can connect" means "can run code in the editor".
...
- **Remote access is a tunnel you own**. The servers never listen on a network address; an editor on another machine is reached by [forwarding its loopback port over SSH](/docs/mcp-setup#reach-an-editor-on-another-machine), which extends "any process on the machine" to anyone who can reach the tunnel's near end.
...
Some situations are different: a shared or untrusted network, or a browser session you do not control. There, quit the editor when you are not driving it, since the servers exist only while it runs. `swallowtail/network/mcp_http` turns the HTTP endpoint off independently, and disabling the plugin stops both.
